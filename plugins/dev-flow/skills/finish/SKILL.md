...
预存脏文件启动时已默认排除，不会阻塞 finalize。任务期间新出现的未知归属必须列出具体文件并 reconcile/回答 ownership decision。真实 stale 时接受 Core 给出的最早恢复阶段，重做受影响步骤，不进行全量无差别重跑。最终交付只包含 Core 从可信写事件、checkpoint、Git 基线与已确认 ownership 派生的 governed 文件。
