...
优先使用 MCP form elicitation 的 `oneOf + const + title` 选项。decline、cancel、协议错误、超时或缺少能力时保留 pending，并提示用户回复选项；文本入口接受完整标签、唯一可判定的简称和 Core 已登记的同义表达，不要要求用户为语义相同的回答反复切换表单或复述整句。含糊简称仍待决。普通 AskUserQuestion/request_user_input 没有可信宿主事件时不能作为接受证据。
