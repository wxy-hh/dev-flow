...
3. 用户拒绝、未回复、没有浏览器工具，或自行验收后没有说明，都不会失败、等待或改变下一步：照常完成 `dev_flow_verify` → feature-check → finalize。
...
finalize 仅依赖通过且 fresh 的机器验证、必需证据、feature-check 与交付快照；可选验收协助从不成为 Core 的硬条件。
