...
先调用 `dev_flow_status`。向用户展示 level、逐项控制理由、完整 `orderedRoute`、当前阶段、待办义务和最早恢复阶段；不要只显示当前一步。review 展示 plan/code 两个 phase 的 current 摘要；history 默认只显示 hot 视图，不要求读冷 evidence payload。需要细节时一次读取一个 `dev_flow_inspect` topic：classification、artifacts、trace、review、implementation、verification、delivery、history 或 diagnostics。
