...
调用 `dev_flow_status`，优先按 `stageCapabilities` 的阶段、能力、完成条件和恢复动作继续；`progress` 只用于查看验证证据和等待详情。禁止直接编辑 `.dev-flow`。intake 显示调查、决策和分类锁定状态；routed 显示当前阶段、待完成义务、Execution Brief 和实际恢复动作。
