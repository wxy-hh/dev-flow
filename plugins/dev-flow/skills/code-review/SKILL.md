...
`independent` 与 `full` 路线要求每个代码审查 job 在与实现隔离的上下文中完成，否则 `record_step code_review` 会被 `REVIEW_ISOLATION_REQUIRED` 阻塞。隔离证明只能来自宿主捕获的审查执行（宿主适配器记录的 review-execution 事件）或受控服务端采样，智能体自行声明或自行写入的事件不构成证明。当前宿主未提供事件接缝时，两种满足方式：
