...
- **1.1.0+** 含 `df-grillme`（需求/方案逐题拷问）；标准 M/L 两态需求在 `requirements` 步骤内强制 grill 子流程，由 core 校验 `grill_status`  
- 技能 id 为短前缀 `df-*`（如 `df-task`）；description 保留旧名 `dev-flow-*` 作匹配兼容  

安装与使用说明见仓库根目录 [README.md](../../README.md)（技能表含 `df-grillme`）。  
