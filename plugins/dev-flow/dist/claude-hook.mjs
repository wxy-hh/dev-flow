...
async function recordNativePromptHealth(root2, event2, host) {
  try {
    await observeHostRecovery(root2, {
      host,
      kind: "user-prompt-submit",
      eventId: `${event2.event_id ?? "native-question"}:answer`
    });
  } catch {
  }
}
...
function questionsFrom(event2) {
  const questions = event2.tool_input?.questions;
...
function structuredAnswers(response, questions) {
  ...
  let root2 = parsed;
  if (root2.data && typeof root2.data === "object" && !Array.isArray(root2.data)) {
    root2 = root2.data;
  }
...
function claudeNativeQuestionAnswers(event2) {
  if (event2.hook_event_name !== "PostToolUse" || event2.tool_name !== "AskUserQuestion") return [];
  const questions = questionsFrom(event2);
  if (!questions.length) return [];
  for (const response of [event2.tool_response, event2.tool_result]) {
...
if (event.hook_event_name === "UserPromptSubmit" || event.hook_event_name === "Stop" || event.hook_event_name === "PostToolUse") {
  if (event.hook_event_name === "PostToolUse") {
    try {
      await recordPermissionPostToolUse(root, event, "claude");
    } catch {
    }
    if (postToolSucceeded(event)) {
      try {
        await recordTrustedWriteOwnership(root, trustedWriteTargets(root, event), "claude", event.event_id ?? event.tool_use_id ?? `post-${Date.now()}`);
      } catch {
      }
