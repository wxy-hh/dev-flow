...
var PolicyError, levels, topologies;
...
function issue(code, path32, message, recoveryHint) {
  return { code, path: path32, message, recoveryHint };
}
...
var levelRank, levelRoute, requiredBoundaryKinds;
var init_route = __esm({
  "plugins/dev-flow/src/policy/route.ts"() {
    ...
    levelRoute = { XS: "xs", S: "s", M: "m", L: "l" };
    requiredBoundaryKinds = ["assumption", "free-space", "tbd", "fallback", "scope", "acceptance"];
  }
});
...
function parseEvidenceStoreCatalog(value) {
  ...
  for (const object2 of objects) {
    if (!packShas.has(object2.packSha256)) throw new TypeError("catalog object references a missing pack");
    const key = `${object2.kind}\0${object2.sha256}`;
...
var init_repository_fact_store = __esm({
  "plugins/dev-flow/src/core/repository-fact-store.ts"() {
    "use strict";
    init_errors();
    digest = (value) => createHash2("sha256").update(value).digest("hex");
  }
});
...
function repositoryFactId(input) {
  return `FACT-${createHash3("sha256").update(`${input.assertion?.trim() ?? ""}
...
function pathWithinFileScope(path32, fileScope) {
  return fileScope.some((pattern) => scopePatternMatches(pattern.normalize("NFC"), path32.normalize("NFC")));
}
...
function isVerificationCommandArray(value) {
  return Array.isArray(value) && value.length > 0 && value.every((item) => {
    ...
    if (!isRecord2(item) || typeof item.command !== "string" || !item.command.trim() || item.args !== void 0 && (!Array.isArray(item.args) || item.args.some((arg) => typeof arg !== "string")) || item.cwd !== void 0 && (typeof item.cwd !== "string" || !item.cwd || item.cwd.startsWith("/") || item.cwd.split(/[\\/]+/).includes(".."))) return false;
    return true;
  });
}
...
function implementationUnitForNode(node, basisHash2) {
  if (!isRecord2(node) || node.kind !== "implementation-unit" || !isImplementationUnitId(node.id) || !isNonEmptyStringArray(node.tasks) || !isNonEmptyStringArray(node.fileScope) || !isVerificationCommandArray(node.forwardVerification) || node.status !== "current") {
    invalid2("implementation unit node is missing fields required to open an implementation unit");
  }
  if (!isSha2562(basisHash2)) invalid2("implementation unit basis hash must be a SHA-256 hex digest");
...
function parseCheckpointManifest(value) {
  ...
  const verificationCommands = value.verificationCommands.map((command2, index) => {
    if (!isRecord2(command2) || !hasOnlyKeys(command2, ["commandId", "command"]) || !isNonEmptyString(command2.commandId) || !isNonEmptyString(command2.command)) {
  ...
  if (value.verificationCommandHashes !== void 0 && (!isRecord2(value.verificationCommandHashes) || Object.entries(value.verificationCommandHashes).some(([id, hash3]) => !declaredCommandIds.has(id) || !isSha2562(hash3)))) {
  ...
  if (value.blobRefs !== void 0) {
    if (value.schemaVersion !== 3 || !isRecord2(value.blobRefs)) invalid2("checkpoint blob refs are only valid on schema v3");
    blobRefs = Object.fromEntries(Object.entries(value.blobRefs).map(([blobSha256, ref]) => {
      if (!isSha2562(blobSha256)) invalid2("checkpoint blob ref key is not a sha256");
      const parsed = parseEvidenceObjectRef(ref);
      if (parsed.kind !== "checkpoint-pack" || parsed.sha256 !== blobSha256) {
        invalid2("checkpoint blob ref does not match its content sha");
      }
  ...
  return {
    ...
    ...value.verificationCommandHashes !== void 0 ? { verificationCommandHashes: Object.fromEntries(Object.entries(value.verificationCommandHashes).map(([id, hash3]) => [id, hash3])) } : {},
...
async function fingerprintGovernedRoots(root2, input) {
  ...
  const digest14 = createHash4("sha256");
  for (const relative of files) {
    const absolute = path4.join(root2, relative);
    const metadata = await lstat2(absolute);
    digest14.update(relative);
    digest14.update("\0");
    if (metadata.isSymbolicLink()) {
      digest14.update("symlink\0");
      digest14.update(await readlink(absolute));
    } else {
      digest14.update("file\0");
      digest14.update(await readFile2(absolute));
    }
    digest14.update("\0");
  }
  return digest14.digest("hex");
}
...
function verificationCommandHashes(config) {
  return Object.fromEntries(config.verification.commands.map((command2) => [
    ...
    createHash5("sha256").update(JSON.stringify({ id: command2.id, command: command2.command, args: command2.args, cwd: command2.cwd })).digest("hex")
  ]));
}
...
function validateNodeInput(value) {
  if (!isRecord3(value) || typeof value.kind !== "string" || !(value.kind in inputKeys)) invalid3("node input has an unknown kind");
  if (value.kind === "rollback") invalid3("rollback nodes are not a v6 Trace kind; use recovery");
...
function validateTraceDelta(value) {
  if (!isRecord3(value) || Object.keys(value).length !== 1 || !Array.isArray(value.nodes)) invalid3("Trace delta must contain only nodes");
  const ids = /* @__PURE__ */ new Set();
...
function assertSourceBlocks(input) {
  ...
  for (const block of input.sourceBlocks) {
    if (!isRecord3(block) || typeof block.id !== "string" || typeof block.kind !== "string" || typeof block.sourceAnchor !== "string" || typeof block.sourceBlockSha256 !== "string") {
...
function assertArtifactDeltaContract(input) {
  ...
  if (!allowed) invalid3("artifact kind is not a v6 Trace source", { artifactKind: input.artifactKind });
  ...
  for (const node of input.delta.nodes) {
    ...
    if (input.artifactKind !== "implementation-plan") invalid3("implementation unit has an invalid source artifact");
...
function assertPersistedNode(recordId, value, options) {
  if (!isRecord3(value)) invalid3("persisted node is not an object", { id: recordId });
  const kind = value.kind;
  ...
  if (kind === "rollback") {
    invalid3("rollback nodes are not a v6 Trace kind; use recovery", { id: recordId });
  }
  if (kind === "implementation-unit") {
...
function validateTraceGraph(ledger, route, mode, options = {}) {
  if (!isRecord3(ledger) || ledger.schemaVersion !== 2 || !isRecord3(ledger.nodes) || !Array.isArray(ledger.edges)) invalid3("traceability ledger has an invalid shape");
...
var ALLOWED_TRACE_KINDS, inputKeys, idPrefix, dispositionKinds, statusValues, sourceArtifacts, hex64;
...
async function writeTraceSnapshot(root2, ledger, options = {}) {
  ...
  const sha2564 = digest2(contents);
  const directory = snapshotDirectory(root2, ledger.featureId);
  const target = path6.join(directory, `${sha2564}.json`);
  ...
  try {
    ...
    const temporary = path6.join(directory, `.${sha2564}.${randomUUID()}.tmp`);
  ...
  return {
    path: `traceability/snapshots/${sha2564}.json`,
    sha256: sha2564,
...
async function readTraceabilityForArtifactReplacement(root2, state, artifactKind) {
  const allowUnsafeFileScopeSourceArtifact = artifactKind === "implementation-plan" ? artifactKind : void 0;
...
async function inspectTraceGate(root2, state, step) {
  ...
  try {
    ...
    const { config, sha256: sha2564 } = await readProjectConfigSnapshot(root2);
    assertTraceSliceCurrent(ledger, state.route, traceStep, sha2564, verificationCommandHashes(config));
    if (traceStep === "implementation_plan") await assertImplementationPlanTraceCurrent(root2, state, ledger);
...
async function assertImplementationPlanTraceCurrent(root2, state, ledger) {
  ...
  const contents = await readFile4(path7.join(root2, ".dev-flow", "features", state.featureId, normalizeUnicode(artifact.path)), "utf8");
...
function parseHostAttestation(value) {
  if (!isRecord5(value) || Object.keys(value).some((key) => !["host", "agentId", "issuedAt", "raw", "hostEventId", "isolated"].includes(key)) || value.host !== "claude" && value.host !== "codex" || typeof value.agentId !== "string" || !value.agentId.trim() || typeof value.issuedAt !== "string" || !value.issuedAt.trim() || Number.isNaN(Date.parse(value.issuedAt)) || typeof value.raw !== "string" || !value.raw.trim()) {
...
function parseReviewJobCompletion(value) {
  if (!isRecord5(value) || Object.keys(value).some((key) => key !== "coverageSummary" && key !== "findings" && key !== "resolutions") || typeof value.coverageSummary !== "string" || !value.coverageSummary.trim() || !Array.isArray(value.findings)) {
...
function parseEvidence(value, label) {
  ...
  return value.map((item, index) => {
    const line = isRecord5(item) ? item.line : void 0;
    if (!isRecord5(item) || Object.keys(item).some((key) => key !== "path" && key !== "line") || typeof item.path !== "string" || !item.path.trim() || line !== void 0 && (typeof line !== "number" || !Number.isInteger(line) || line < 1)) {
...
function parseFinding(value, index) {
  if (!isRecord5(value) || Object.keys(value).some((key) => !["severity", "category", "targets", "evidence", "claim", "recommendation"].includes(key)) || value.severity !== "blocking" && value.severity !== "warning" && value.severity !== "note" || !isReviewRole(value.category) || !nonEmptyStrings(value.targets) || typeof value.claim !== "string" || !value.claim.trim() || typeof value.recommendation !== "string" || !value.recommendation.trim()) {
...
function parseResolution(value, index) {
  if (!isRecord5(value) || Object.keys(value).some((key) => key !== "findingId" && key !== "evidence" && key !== "note" && key !== "outcome") || typeof value.findingId !== "string" || !value.findingId || typeof value.note !== "string" || !value.note.trim()) {
...
var defaultReviewIdentityVerifier, reviewRoles2;
...
import { mkdir as mkdir2, open as open2, readFile as readFile5, readdir as readdir4, rename as rename2 } from "node:fs/promises";
...
function snapshotDirectory2(root2, featureId) {
  return path8.join(root2, ".dev-flow", "features", featureId, "review", "snapshots");
}
function packageDirectory(root2, featureId) {
  return path8.join(root2, ".dev-flow", "features", featureId, "review", "packages");
}
...
function validAttestation(value) {
  return isRecord6(value) && (value.host === "claude" || value.host === "codex") && typeof value.agentId === "string" && value.agentId.trim().length > 0 && typeof value.issuedAt === "string" && !Number.isNaN(Date.parse(value.issuedAt)) && typeof value.raw === "string" && value.raw.trim().length > 0 && validHash(value.rawSha256) && typeof value.acceptedAt === "string" && digest3(value.raw) === value.rawSha256;
}
...
async function writeReviewSnapshot(root2, ledger) {
  ...
  const target = path8.join(directory, `${sha2564}.json`);
  ...
  try {
    ...
    if (error.code !== "ENOENT") throw error;
    const temporary = path8.join(directory, `.${sha2564}.${randomUUID2()}.tmp`);
    ...
    try {
      ...
      if (renameError.code !== "EEXIST") throw renameError;
      if (await readFile5(target, "utf8") !== contents) integrity2("concurrent review snapshot does not match its content address");
    }
    await fsyncDirectory2(directory);
  }
  return { path: `review/snapshots/${sha2564}.json`, sha256: sha2564, revision: ledger.revision, summary: ledger.summary };
}
async function writeReviewPackage(root2, featureId, value) {
  ...
  try {
    ...
    try {
      ...
      if (renameError.code !== "EEXIST") throw renameError;
      if (await readFile5(target, "utf8") !== contents) integrity2("concurrent review package does not match its content address");
    }
    await fsyncDirectory2(directory);
  }
  return sha2564;
}
async function readReviewPackage(root2, featureId, sha2564) {
  if (!validHash(sha2564)) integrity2("review package hash is invalid");
  let contents;
  try {
    contents = await readFile5(path8.join(packageDirectory(root2, featureId), `${sha2564}.json`), "utf8");
  } catch {
    integrity2("review package cannot be read", { featureId, sha256: sha2564 });
  }
  if (digest3(contents) !== sha2564) integrity2("review package digest does not match its address", { featureId, sha256: sha2564 });
  try {
    return JSON.parse(contents);
  } catch {
    integrity2("review package is not valid JSON", { featureId, sha256: sha2564 });
  }
}
...
async function readReviewLedger(root2, state) {
  ...
  let contents;
  try {
    contents = await readFile5(path8.join(root2, ".dev-flow", "features", state.featureId, relative), "utf8");
  } catch {
...
var emptySummary, digest3;
var init_review_store = __esm({
  "plugins/dev-flow/src/core/review-store.ts"() {
    "use strict";
    init_review();
    init_errors();
    emptySummary = () => ({ batches: 0, current: 0, stale: 0, open: 0, complete: 0 });
    digest3 = (contents) => createHash8("sha256").update(contents).digest("hex");
  }
});
...
async function writeFileAtomic(target, bytes) {
  ...
  const temporary = path9.join(path9.dirname(target), `.${path9.basename(target)}.${randomUUID3()}.tmp`);
  ...
  await fsyncDirectory3(path9.dirname(target));
}
...
async function writeEvidencePack(directory, inputs, options = {}) {
  ...
  const packPath = path9.join(directory, `${packSha256}.pack`);
  const indexPath = path9.join(directory, `${packSha256}.index.json`);
  await mkdir3(directory, { recursive: true });
  try {
    const existingPack = await readFile6(packPath);
    ...
    const packTemporary = path9.join(directory, `.${packSha256}.${randomUUID3()}.tmp`);
  ...
  try {
    const existingIndex = await readFile6(indexPath, "utf8");
...
async function readEvidencePackEntry(directory, packSha256, entry) {
  const pack = await readFile6(path9.join(directory, `${packSha256}.pack`));
...
import { copyFile, mkdir as mkdir4, open as open4, readFile as readFile7, readdir as readdir5, rename as rename4, stat, unlink } from "node:fs/promises";
import path10 from "node:path";
function evidenceDirectory(root2, featureId) {
  return path10.join(root2, ".dev-flow", "features", featureId, "evidence");
}
function hotDirectory(root2, featureId) {
  return path10.join(evidenceDirectory(root2, featureId), "packs", "hot");
}
function coldDirectory(root2, featureId) {
  return path10.join(evidenceDirectory(root2, featureId), "packs", "cold");
}
function catalogPath(root2, featureId) {
  return path10.join(evidenceDirectory(root2, featureId), "catalog.json");
}
function packDirectory(root2, featureId, descriptor) {
  return descriptor.location === "cold" ? coldDirectory(root2, featureId) : hotDirectory(root2, featureId);
}
function emptyCatalog(featureId) {
  return { schemaVersion: 1, featureId, revision: 0, objects: [], packs: [] };
}
function digest4(contents) {
  return createHash10("sha256").update(contents).digest("hex");
}
...
async function writeFileAtomic2(target, contents) {
  ...
  const temporary = path10.join(path10.dirname(target), `.${path10.basename(target)}.${randomUUID4()}.tmp`);
  ...
  await fsyncDirectory4(path10.dirname(target));
}
async function readEvidenceStoreCatalog(root2, featureId) {
  let contents;
  try {
    contents = await readFile7(catalogPath(root2, featureId), "utf8");
  } catch (error) {
...
async function writeEvidenceStoreCatalog(root2, featureId, catalog, options = {}) {
  const contents = canonicalCatalogJson(catalog);
  const sha2564 = digest4(contents);
  await mkdir4(evidenceDirectory(root2, featureId), { recursive: true });
  await options.fault?.("before-catalog-write");
  await writeFileAtomic2(catalogPath(root2, featureId), contents);
...
async function putEvidenceObjects(root2, featureId, inputs, options = {}) {
  ...
  for (const input of inputs) {
    ...
    const sha2564 = digest4(input.bytes);
  ...
  const current = await readEvidenceStoreCatalog(root2, featureId);
  const existingByKey = new Map(current.objects.map((entry) => [objectKey(entry), entry]));
  const missing = normalized.filter((input) => !existingByKey.has(objectKey({ kind: input.kind, sha256: digest4(input.bytes) })));
  const refs = normalized.map((input) => {
    const sha2564 = digest4(input.bytes);
    return { kind: input.kind, sha256: sha2564, size: input.bytes.length };
  });
  if (missing.length === 0) {
    const sha2564 = digest4(canonicalCatalogJson(current));
    return { refs, pointer: evidenceStorePointer(current, sha2564), catalog: current };
  }
  const pack = await writeEvidencePack(hotDirectory(root2, featureId), missing, options);
  const descriptor = {
  ...
  const written = await writeEvidenceStoreCatalog(root2, featureId, next, options);
  return { refs, pointer: evidenceStorePointer(written.catalog, written.sha256), catalog: written.catalog };
}
async function putEvidenceObject(root2, featureId, kind, canonicalBytes, options = {}) {
  const bytes = Buffer.isBuffer(canonicalBytes) ? canonicalBytes : Buffer.from(canonicalBytes, "utf8");
  const result = await putEvidenceObjects(root2, featureId, [{ kind, bytes }], options);
  return { ref: result.refs[0], ...result };
}
async function readEvidenceObject(root2, featureId, ref) {
  parseEvidenceObjectRef(ref);
  const catalog = await readEvidenceStoreCatalog(root2, featureId);
  ...
  const directory = packDirectory(root2, featureId, descriptor);
  let indexJson;
  try {
    indexJson = await readFile7(path10.join(directory, `${descriptor.packSha256}.index.json`), "utf8");
  } catch {
  ...
  if (digest4(indexJson) !== descriptor.indexSha256) integrity3("evidence pack index digest does not match catalog", { featureId, packSha256: descriptor.packSha256 });
...
async function collectEvidenceOrphans(root2, featureId, rootSet, options = {}) {
  const current = await readEvidenceStoreCatalog(root2, featureId);
  ...
  if (selected.length > 0) {
    ...
    await writeEvidenceStoreCatalog(root2, featureId, next, options);
  }
  let deletedFiles = 0;
  let deletedBytes = 0;
  for (const pack of selected) {
    const directory = packDirectory(root2, featureId, pack);
    for (const suffix of [`${pack.packSha256}.pack`, `${pack.packSha256}.index.json`]) {
      const file = path10.join(directory, suffix);
  ...
  for (const directory of [hotDirectory(root2, featureId), coldDirectory(root2, featureId)]) {
    let files;
    try {
      files = await readdir5(directory);
    } catch (error) {
      if (error.code === "ENOENT") continue;
      throw error;
    }
    for (const file of files) {
      ...
      const absolute = path10.join(directory, file);
...
async function inspectEvidenceStoreHealth(root2, featureId, options = {}) {
  const catalog = await readEvidenceStoreCatalog(root2, featureId);
  ...
  const rootPackShas = /* @__PURE__ */ new Set();
  ...
  const objectsByPack = /* @__PURE__ */ new Map();
  const rawBytesByPack = /* @__PURE__ */ new Map();
  ...
  for (const pack of catalog.packs) {
    ...
    const missing = [];
    for (const suffix of [`${pack.packSha256}.pack`, `${pack.packSha256}.index.json`]) {
      const exists = await stat(path10.join(directory, suffix)).then(() => true, (error) => error.code === "ENOENT" ? false : Promise.reject(error));
...
import { mkdir as mkdir5, open as open5, readFile as readFile8, rename as rename5, writeFile } from "node:fs/promises";
import path11 from "node:path";
async function readIndex(root2, featureId) {
  try {
    const raw = await readFile8(segmentIndexPath(root2, featureId), "utf8");
...
async function writeIndex(root2, featureId, index) {
  const directory = path11.dirname(segmentIndexPath(root2, featureId));
  await mkdir5(directory, { recursive: true });
  const temporary = path11.join(directory, `.segment-index.${randomUUID5()}.tmp`);
...
`);
  ...
  await rename5(temporary, segmentIndexPath(root2, featureId));
}
async function readHotRecords(root2, featureId, options = {}) {
  let raw;
  try {
    raw = await readFile8(hotPath(root2, featureId), "utf8");
  } catch (error) {
    if (error.code === "ENOENT") return [];
    throw error;
  }
...
async function nextFeatureEventSequence(root2, featureId) {
  const index = await readIndex(root2, featureId);
  const previous = index.entries.at(-1)?.lastSequence ?? 0;
  const hot = await readHotRecords(root2, featureId, { startSequence: previous });
  return (hot.at(-1)?.eventSequence ?? previous) + 1;
}
async function maybeSealFeatureEvents(root2, featureId) {
  const hot = await readHotRecords(root2, featureId, { startSequence: (await readIndex(root2, featureId)).entries.at(-1)?.lastSequence ?? 0 });
  if (hot.length === 0) return void 0;
  return sealFeatureEvents(root2, featureId);
}
async function sealFeatureEvents(root2, featureId) {
  const index = await readIndex(root2, featureId);
  const previous = index.entries.at(-1);
  const firstSequence = previous ? previous.lastSequence + 1 : 1;
  const hot = await readHotRecords(root2, featureId, { startSequence: previous ? previous.lastSequence : 0 });
  ...
  const stored = await putEvidenceObject(root2, featureId, "event-segment", Buffer.from(`${stableJson(segment)}
`, "utf8"));
  const nextIndex = {
    schemaVersion: 1,
    featureId,
  ...
  await writeIndex(root2, featureId, nextIndex);
  await writeFile(hotPath(root2, featureId), "");
  return { ref: stored.ref, segment, sealed: records.length };
}
async function readSegmentedFeatureEvents(root2, featureId, options = {}) {
  const index = await readIndex(root2, featureId);
  const records = [];
  for (const entry of index.entries) {
    if (entry.lastSequence <= (options.afterSequence ?? 0)) continue;
    const bytes = await readEvidenceObject(root2, featureId, entry.ref);
  ...
  const hot = await readHotRecords(root2, featureId, { startSequence: base });
  records.push(...hot);
...
async function eventSegmentRootRefs(root2, featureId) {
  const index = await readIndex(root2, featureId);
...
var init_event_segments = __esm({
  "plugins/dev-flow/src/core/event-segments.ts"() {
    ...
    featureDirectory = (root2, id) => path11.join(root2, ".dev-flow", "features", id);
    hotPath = (root2, id) => path11.join(featureDirectory(root2, id), "events.jsonl");
    segmentIndexPath = (root2, id) => path11.join(featureDirectory(root2, id), "events", "segment-index.json");
  }
});
...
async function archiveLargeStateCollections(root2, state) {
...
async function readArchivedJson(root2, featureId, ref) {
  if (ref === void 0) return void 0;
  return JSON.parse((await readEvidenceObject(root2, featureId, ref)).toString("utf8"));
}
async function hydrateFeatureState(root2, raw) {
  ...
  if (pointers.workspaceLineage) {
    state.workspace = parseWorkspaceLineage(await readArchivedJson(root2, featureId, pointers.workspaceLineage));
  }
  if (pointers.governanceLedger) {
    state.governance = parseGovernanceLedger(await readArchivedJson(root2, featureId, pointers.governanceLedger));
  }
  if (pointers.interactionLedger) {
    const resolvedInteractions = parseInteractionLedger(await readArchivedJson(root2, featureId, pointers.interactionLedger));
    state.interactions = { ...state.interactions ?? {}, ...resolvedInteractions };
  }
  const verificationLedger = pointers.verificationLedger ? parseAttemptLog(await readArchivedJson(root2, featureId, pointers.verificationLedger), "verification") : void 0;
  state.verification = {
  ...
  if (pointers.repairLedger) {
    const repairLedger = parseAttemptLog(await readArchivedJson(root2, featureId, pointers.repairLedger), "repair");
    state.repair = { ...state.repair ?? { status: "completed", maxAttempts: 3 }, attempts: repairLedger };
  }
  return state;
}
async function persistableFeatureState(root2, state) {
  const pointers = await archiveLargeStateCollections(root2, state);
...
import { readFile as readFile9, readdir as readdir6 } from "node:fs/promises";
...
async function checkpointManifestRootRefs(root2, featureId) {
  const directory = path12.join(root2, ".dev-flow", "features", featureId, "checkpoints", "manifests");
  let files;
  try {
    files = (await readdir6(directory)).filter((file) => file.endsWith(".json"));
  } catch (error) {
    if (error.code === "ENOENT") return [];
    throw error;
  }
  ...
  for (const file of files) {
    ...
    const manifest = await readCheckpointManifest(root2, featureId, checkpointId);
...
function projectionDirectory(root2, featureId) {
  return path13.join(root2, ".dev-flow", "features", featureId, "review", "projections");
}
async function fsyncDirectory5(directory) {
...
async function writeProjection(root2, featureId, markdown) {
  const sha2564 = digest5(markdown);
  const directory = projectionDirectory(root2, featureId);
  const target = path13.join(directory, `${sha2564}.md`);
  ...
  try {
    ...
    if (error.code !== "ENOENT") throw error;
    const temporary = path13.join(directory, `.${sha2564}.${randomUUID6()}.tmp`);
    ...
    await fsyncDirectory5(directory);
  }
...
async function readReviewProjection(root2, state) {
  ...
  let markdown;
  try {
    markdown = await readFile10(path13.join(root2, ".dev-flow", "features", state.featureId, artifact.path), "utf8");
  } catch {
    projectionError("review projection artifact cannot be read", { featureId: state.featureId, path: artifact.path });
  }
  if (digest5(markdown) !== artifact.sha256) projectionError("review projection digest does not match artifact pointer", { featureId: state.featureId });
  const ledger = await readReviewLedger(root2, state);
  const model = reviewProjectionModel(state, ledger);
...
var init_review_projection = __esm({
  "plugins/dev-flow/src/core/review-projection.ts"() {
    ...
    digest5 = (contents) => createHash12("sha256").update(contents).digest("hex");
  }
});
...
var confirmationTerms, optionAliases, interactionAliases;
var init_decision_language = __esm({
  "plugins/dev-flow/src/core/decision-language.ts"() {
    "use strict";
    init_text_normalization();
    confirmationTerms = ["\u786E\u8BA4", "\u540C\u610F", "\u6279\u51C6", "\u53EF\u4EE5", "\u597D\u7684", "\u884C", "\u6CA1\u95EE\u9898", "lgtm", "approved"];
    optionAliases = {
...
function createInteraction(state, input) {
  ...
  const interaction = {
    ...
    presentationEventId: input.presentationEventId ?? randomUUID7(),
...
import { lstat as lstat3, readFile as readFile11, readlink as readlink2 } from "node:fs/promises";
import path14 from "node:path";
async function captureWorkspaceSnapshot(root2, featureId, input, options = {}) {
  const paths = await enumerateProtectedFiles(root2, input);
  const files = [];
  for (const relative of paths) {
    const absolute = path14.join(root2, relative);
    const metadata = await lstat3(absolute);
    const symbolic = metadata.isSymbolicLink();
    const bytes = symbolic ? Buffer.from(await readlink2(absolute)) : await readFile11(absolute);
    files.push({
      path: relative,
...
function featureOwnedContentHash(snapshot, ownership) {
  return featureOwnedSnapshotHash(snapshot.files, ownership);
}
function emptyHash() {
  return "0".repeat(64);
}
async function captureEvidenceBaseline(root2, state, config, origin, options = {}) {
  const snapshot = await captureWorkspaceSnapshot(root2, state.featureId, config, { now: options.now });
  const units = state.implementationUnits ?? [];
  const fileToUnits = /* @__PURE__ */ new Map();
  const checkpointIds = [];
  for (const unit of units) {
    if (!unit.checkpointId) continue;
    checkpointIds.push(unit.checkpointId);
    const manifest2 = await readCheckpointManifest(root2, state.featureId, unit.checkpointId);
    for (const file of manifest2.files) {
      const unitIds = fileToUnits.get(file.path) ?? /* @__PURE__ */ new Set();
  ...
  const manifest = {
    ...
    governedScopeHash: sha2562(stableJson({ governedRoots: config.governedRoots, excludes: config.governedRootsExclude ?? [] })),
    ownershipHash: sha2562(stableJson(state.workspace.ownership)),
    planExecutionBasisHash: state.executionSemanticBasisHash ?? emptyHash(),
    checkpointIds: [...new Set(checkpointIds)].sort(),
    fileToUnits: [...fileToUnits.entries()].map(([filePath, unitIds]) => ({ path: filePath, unitIds: [...unitIds].sort() })).sort((left, right) => left.path.localeCompare(right.path)),
...
var init_evidence_baseline2 = __esm({
  "plugins/dev-flow/src/core/evidence-baseline.ts"() {
    ...
    sha2562 = (value) => createHash14("sha256").update(value).digest("hex");
  }
});
    ...
    return {
      mutate: async (draft) => {
        response = resolveResponseForAnswer(draft, interaction, {
          ...
          userReply: credential.source === "text" ? credential.promptText : void 0,
          promptText,
          promptEventId,
          host
        });
        ...
        if (response.action === "accept") {
          const config = await readProjectConfig(root2);
          const baseline = await captureEvidenceBaseline(root2, draft, config, {
          ...
          const authorizationId = `AUTH-${createHash15("sha256").update(`${kind}|${fingerprint2}|${response.respondedAt}`).digest("hex").slice(0, 16)}`;
...
__export(review_jobs_exports, {
  assertReviewRiskAcceptanceEvidence: () => assertReviewRiskAcceptanceEvidence,
  beginReviewSampling: () => beginReviewSampling,
  claimReviewJob: () => claimReviewJob,
  codeReviewIsolationRequired: () => codeReviewIsolationRequired,
  completeReviewSampling: () => completeReviewSampling,
  createReviewBatch: () => createReviewBatch,
  currentUnresolvedBlocking: () => currentUnresolvedBlocking,
  failReviewSampling: () => failReviewSampling,
  getReviewJob: () => getReviewJob,
  jobHasEffectiveIsolationProof: () => jobHasEffectiveIsolationProof,
  presentReviewRiskAcceptance: () => presentReviewRiskAcceptance,
  releaseReviewJob: () => releaseReviewJob,
  requireReviewReady: () => requireReviewReady,
  resolveReviewRiskAcceptanceForAnswer: () => resolveReviewRiskAcceptanceForAnswer,
  reviewGate: () => reviewGate,
  roleBasisHash: () => roleBasisHash,
  staleCurrentBatchesOfPhase: () => staleCurrentBatchesOfPhase,
  startIsolatedReview: () => startIsolatedReview,
  submitParsedReviewJob: () => submitParsedReviewJob,
  submitReviewJob: () => submitReviewJob
});
...
function roleBasisHash(basis, frozenArtifacts, trace2, role) {
  ...
  const artifacts2 = frozenArtifacts.filter((artifact) => spec.artifactKinds.includes(artifact.kind)).map(({ kind, path: artifactPath, sha256: sha2564 }) => ({ kind, path: artifactPath, sha256: sha2564 }));
  const traceSlice = Object.values(trace2?.nodes ?? {}).filter((node) => node.status !== "tombstoned" && spec.traceKinds.includes(node.kind)).sort((left, right) => left.id.localeCompare(right.id)).map(({ sourceArtifact: _sourceArtifact, sourceSha256: _sourceSha256, sourceAnchor: _sourceAnchor, sourceBlockSha256: _sourceBlockSha256, status: _status, ...semantic }) => semantic);
  if (spec.riskLabels) {
  ...
  return digest6(canonicalReviewValueJson({
    ...
    ...artifacts2.length ? { artifacts: artifacts2 } : {},
    traceSlice,
    ...spec.bindFeatureOwnedContent ? { featureOwnedFingerprint: basis.featureOwnedFingerprint ?? "" } : {},
    ...spec.bindNonBehaviorDispositions ? { nonBehaviorDispositions: nonBehaviorDispositions(trace2) } : {},
...
function assertFindingScope(manifest, findings, resolutions) {
  ...
  for (const finding of findings) {
    ...
    invalidPaths.push(...finding.evidence.map((evidence) => evidence.path).filter((path32) => !validEvidence(path32)));
  }
  invalidPaths.push(...resolutions.flatMap((resolution) => resolution.evidence.map((evidence) => evidence.path).filter((path32) => !validEvidence(path32))));
...
async function startIsolatedReview(root2, id, expectedRevision, batchId, jobId, executionId, host) {
  ...
  const declarationId = randomUUID8();
...
function staleCurrentBatchesOfPhase(batches, phase) {
  return batches.map((candidate) => candidate.validity === "current" && candidate.phase === phase ? { ...candidate, validity: "stale" } : candidate);
}
async function createReviewBatch(root2, id, expectedRevision) {
  ...
  const state = await mutatePrepared(root2, id, expectedRevision, "review-batch-created", async (current, nextStateRevision) => {
    ...
    if (currentTrace) await assertImplementationPlanTraceCurrent(root2, current, currentTrace);
...
async function resolveReviewRiskAcceptanceForAnswer(ctx) {
  ...
  if (credential.source === "text") {
    ...
    const hostEvent = events.find((event) => event.type === "host-event" && event.data.eventId === promptEventId);
  ...
  const next = await mutatePrepared(root2, featureId, expectedRevision, "review-risk-acceptance-resolved", async (current, nextStateRevision) => {
    ...
    if (live.status === "resolved") {
      ...
      const accepted = credential.source === "text" ? live.response?.action === "accept" && live.response.source === "text" && live.response.userReply === promptText && live.response.promptEventId === promptEventId && live.response.host === host : live.response?.action === "accept" && live.response.source === "elicitation" && live.response.host === host;
    ...
    const resolveOn = (draft) => resolveResponseForAnswer(draft, interaction, {
      ...
      action: credential.source === "elicitation" ? credential.action : void 0,
      comment: credential.source === "elicitation" ? credential.comment : void 0,
      userReply: credential.source === "text" ? promptText : void 0,
      promptText,
      promptEventId,
      host
    });
...
var digest6, leaseMilliseconds, samplingLeaseMilliseconds, basisArtifactKinds, severityRank;
var init_review_jobs = __esm({
  "plugins/dev-flow/src/core/review-jobs.ts"() {
    ...
    severityRank = { note: 0, warning: 1, blocking: 2 };
  }
});
...
import { mkdir as mkdir7, open as open7, readFile as readFile12, rename as rename7 } from "node:fs/promises";
import path15 from "node:path";
async function captureHostReviewEnvelope(root2, input) {
  ...
  const rawStored = await putEvidenceObject(root2, input.featureId, "review-result", rawBytes);
  const envelope = {
    schemaVersion: 1,
  ...
  const envelopeStored = await putEvidenceObject(root2, input.featureId, "review-result", Buffer.from(`${stableJson(envelope)}
`, "utf8"));
  parseReviewResultEnvelope(envelope);
  return { ref: envelopeStored.ref, envelope };
}
async function readHostReviewEnvelope(root2, featureId, ref) {
  const bytes = await readEvidenceObject(root2, featureId, ref);
  let envelope;
  try {
...
function executionDirectory(root2, featureId) {
  return path15.join(root2, ".dev-flow", "features", featureId, "review", "executions");
}
function executionIndexPath(root2, featureId) {
  return path15.join(executionDirectory(root2, featureId), "index.json");
}
async function readExecutionIndex(root2, featureId) {
  try {
    const raw = await readFile12(executionIndexPath(root2, featureId), "utf8");
...
async function writeExecutionIndex(root2, featureId, index) {
  const directory = executionDirectory(root2, featureId);
  await mkdir7(directory, { recursive: true });
  const target = executionIndexPath(root2, featureId);
  const temporary = path15.join(directory, `.index.${randomUUID9()}.tmp`);
...
async function writeReviewExecutionRecord(root2, featureId, record) {
  const stored = await putEvidenceObject(root2, featureId, "review-execution", Buffer.from(`${stableJson(record)}
`, "utf8"));
  return { ref: stored.ref, pointer: stored.pointer };
}
async function readReviewExecutionRecord(root2, featureId, executionRequestId) {
  const index = await readExecutionIndex(root2, featureId);
...
function isolationProofForEnvelope(envelope, events) {
  if (envelope.source !== "claude-subagent" || !envelope.hostEventId || !envelope.parentContext || !envelope.childContext) return void 0;
  if (envelope.parentContext === envelope.childContext) return void 0;
  const proofEvent = events.find((event) => {
    const data = event.data;
    return event.type === "review-execution" && data?.type === "review-execution" && data.eventId === envelope.hostEventId && data.host === envelope.host && data.contextId === envelope.childContext && data.implementationContextId === envelope.parentContext;
  });
  return proofEvent ? { mode: "subagent", hostEventId: envelope.hostEventId } : void 0;
}
async function startReviewExecution(root2, featureId, expectedRevision, batchId, host, executionRequestId, options = {}) {
  requireExecutionRequestId(executionRequestId);
  const resolved = options instanceof Date ? { now: options } : options;
  const now = resolved.now ?? /* @__PURE__ */ new Date();
  ...
  const existingIndex = await readExecutionIndex(root2, featureId);
  const existingEntry = existingIndex.entries.find((entry) => entry.executionRequestId === executionRequestId);
  let result;
  const state = await mutatePrepared(root2, featureId, expectedRevision, "review-execution-started", async (current, nextStateRevision) => {
    const ledger = await readReviewLedger(root2, current);
    ...
    if (existingEntry) {
      ...
      return { mutate: () => void 0, unchanged: true, eventData: { executionRequestId, idempotent: true } };
    }
    const leases = pending.map((job) => {
      const capability = `${executionRequestId}:${job.jobId}:${randomUUID9()}`;
      return {
        job,
        capability,
        declarationId: randomUUID9(),
        lease: {
          jobId: job.jobId,
          role: job.role,
          capabilityHash: sha2563(capability),
          declarationId: "",
          packageSha256: job.packageSha256,
          leaseGeneration: 1,
          leasedAt: now.toISOString(),
          leaseExpiresAt: new Date(now.getTime() + leaseMilliseconds2).toISOString(),
          state: "leased"
        }
      };
    });
    for (const lease of leases) {
      await appendFeatureEvent(root2, featureId, current.revision, "review-execution-declared", {
        type: "review-execution-declared",
    ...
    const batches = ledger.batches.map((candidate) => candidate.batchId !== batchId ? candidate : {
      ...
      jobs: batch.jobs.map((job) => {
        const lease = leases.find((candidate2) => candidate2.job.jobId === job.jobId);
        if (!lease) return job;
        return {
          ...job,
          status: "claimed",
          claim: {
            requestSha256: sha2563(lease.capability),
    ...
    const pointer = await writeReviewSnapshot(root2, {
      ...ledger,
      revision: ledger.revision + 1,
      stateRevision: nextStateRevision,
      batches,
    ...
    const record = {
      ...
      batchId,
      executionRequestId,
      source: useSampling ? "server-sampling" : "claude-subagent",
      host,
    ...
    const stored = await writeReviewExecutionRecord(root2, featureId, record);
    const nextIndex = {
      ...existingIndex,
      entries: [
    ...
    await writeExecutionIndex(root2, featureId, nextIndex);
    result = {
      batchId,
      executionRequestId,
  ...
  if (useSampling && resolved.sampleReview && result && !result.idempotent) {
    const { getReviewJob: getReviewJob2 } = await Promise.resolve().then(() => (init_review_jobs(), review_jobs_exports));
    const sampledJobs = await Promise.all(result.jobs.map(async (job) => {
      const packed = await getReviewJob2(root2, featureId, batchId, job.jobId, job.capability);
...
`, "utf8");
      const captured = await captureHostReviewEnvelope(root2, {
        featureId,
        batchId,
        jobId: job.jobId,
        role: job.role,
        packageSha256: job.packageSha256,
        capabilityHash: sha2563(job.capability),
        executionRequestId,
        leaseGeneration: 1,
      ...
      await recordCapturedEnvelope(root2, featureId, executionRequestId, captured.ref);
    }
  }
  return { ...result, state };
}
async function recordCapturedEnvelope(root2, featureId, executionRequestId, envelopeRef) {
  const index = await readExecutionIndex(root2, featureId);
  const entry = index.entries.find((candidate) => candidate.executionRequestId === executionRequestId);
  if (!entry) throw new DevFlowError("REVIEW_EXECUTION_NOT_FOUND", "review execution record not found", { executionRequestId });
  const record = await readReviewExecutionRecord(root2, featureId, executionRequestId);
  ...
  const stored = await writeReviewExecutionRecord(root2, featureId, updated);
  await writeExecutionIndex(root2, featureId, {
    ...index,
    entries: index.entries.map((candidate) => candidate.executionRequestId === executionRequestId ? { ...candidate, ref: stored.ref } : candidate)
  });
}
async function reviewExecutionEvidenceRoots(root2, featureId) {
  const index = await readExecutionIndex(root2, featureId);
  const refs = [];
  for (const entry of index.entries) {
    if (refs.some((candidate) => candidate.kind === entry.ref.kind && candidate.sha256 === entry.ref.sha256)) continue;
    refs.push(entry.ref);
    const record = await readReviewExecutionRecord(root2, featureId, entry.executionRequestId);
    for (const envelopeRef of record.envelopes) {
      ...
      const envelope = await readHostReviewEnvelope(root2, featureId, envelopeRef);
...
async function completeReviewExecution(root2, featureId, expectedRevision, batchId, executionRequestId, now = /* @__PURE__ */ new Date()) {
  const record = await readReviewExecutionRecord(root2, featureId, executionRequestId);
  ...
  let submittedJobIds = [];
  let completed;
  const state = await mutatePrepared(root2, featureId, expectedRevision, "review-execution-completed", async (current, nextStateRevision) => {
    const ledger = await readReviewLedger(root2, current);
    ...
    for (const ref of record.envelopes) {
      const envelope = await readHostReviewEnvelope(root2, featureId, ref);
      ...
      const raw = await readEvidenceObject(root2, featureId, envelope.rawResultRef);
      let parsed;
      try {
    ...
    const updatedRecord = {
      ...
      leases: record.leases.map((lease) => {
        ...
        return { ...lease, state: job?.status === "submitted" ? "submitted" : lease.state };
      }),
      envelopes: record.envelopes
    };
    const stored = await writeReviewExecutionRecord(root2, featureId, updatedRecord);
    await writeExecutionIndex(root2, featureId, {
      ...await readExecutionIndex(root2, featureId),
      entries: (await readExecutionIndex(root2, featureId)).entries.map((entry) => entry.executionRequestId === executionRequestId ? { ...entry, ref: stored.ref } : entry)
    });
    completed = batch;
    return {
      mutate: (draft) => {
...
var init_review_execution2 = __esm({
  "plugins/dev-flow/src/core/review-execution.ts"() {
    ...
    init_state_store();
    init_obligations();
    sha2563 = (bytes) => createHash17("sha256").update(bytes).digest("hex");
...
async function collectEvidenceRootSet(root2, featureId, state) {
  ...
  for (const ref of await eventSegmentRootRefs(root2, featureId)) pushUnique(ref);
  for (const ref of await checkpointManifestRootRefs(root2, featureId)) pushUnique(ref);
  for (const ref of await reviewExecutionEvidenceRoots(root2, featureId)) pushUnique(ref);
  for (const claim of state.governance?.claims ?? []) {
    if (!claim.baselineRef) continue;
    try {
      const manifestBytes = await readEvidenceObject(root2, featureId, claim.baselineRef);
  ...
  for (const authorization of state.governance?.authorizations ?? []) {
    ...
    try {
      const manifestBytes = await readEvidenceObject(root2, featureId, authorization.baselineRef);
      const manifest = parseEvidenceBaselineManifest(JSON.parse(manifestBytes.toString("utf8")));
      pushUnique(manifest.snapshotRef);
    } catch {
    }
  ...
  for (const interaction of Object.values(state.interactions ?? {})) {
    ...
    try {
      const proposalBytes = await readEvidenceObject(root2, featureId, interaction.planRevisionProposal);
...
async function runBoundedEvidenceMaintenance(root2, featureId, state, options = {}) {
  const roots = await collectEvidenceRootSet(root2, featureId, state);
  const result = await collectEvidenceOrphans(root2, featureId, roots, {
...
import { lstat as lstat4, readFile as readFile13, readlink as readlink3 } from "node:fs/promises";
import path16 from "node:path";
import { createHash as createHash18 } from "node:crypto";
async function trustedWriteSummary(root2, file) {
  const target = path16.join(root2, file);
  try {
    const metadata = await lstat4(target);
    const bytes = metadata.isSymbolicLink() ? Buffer.from(await readlink3(target)) : await readFile13(target);
    return `${metadata.isSymbolicLink() ? "symlink" : "file"}:${createHash18("sha256").update(bytes).digest("hex")}`;
  } catch (error) {
...
import { lstat as lstat5, readFile as readFile14 } from "node:fs/promises";
...
async function contentHash(root2, relative) {
  try {
    const metadata = await lstat5(path17.join(root2, relative));
    if (!metadata.isFile()) return void 0;
    return createHash19("sha256").update(await readFile14(path17.join(root2, relative))).digest("hex");
  } catch {
    return void 0;
  }
}
...
async function captureObservedCommits(root2, baseHead, observedHead) {
  ...
  for (const line of output.split("\n").filter(Boolean)) {
    const [hash3, parents = ""] = line.split("\0");
    if (!hash3) continue;
    const paths = await git(root2, ["show", "--format=", "--name-only", "--pretty=", hash3]);
    commits.push({
      hash: hash3,
...
function matchDecisionReply(decision, userReply) {
  ...
  let match;
  const answerCodeMatch = options.find((option) => option.answerCode && normalizeReplyText(option.answerCode) === normalized);
...
import { mkdir as mkdir8, open as open8, readFile as readFile15 } from "node:fs/promises";
import path18 from "node:path";
async function readHostHealth(root2) {
  try {
    const raw = await readFile15(hostHealthPath(root2), "utf8");
...
var init_host_health = __esm({
  "plugins/dev-flow/src/core/host-health.ts"() {
    ...
    hostHealthPath = (root2) => path18.join(root2, ".dev-flow", "host-health.jsonl");
  }
});
...
async function openQuestionsAdvisory(root2, state) {
  ...
  try {
    ...
    const contents = await readFile16(
      path19.join(root2, ".dev-flow", "features", state.featureId, normalizeUnicode(artifact.path)),
      "utf8"
    );
...
async function requestGrillDecision(root2, id, expectedRevision, input) {
  ...
  const state = await mutate(root2, id, expectedRevision, "decision-presented", (draft) => {
    ...
    draft.lastUpdatedBy = { host: input.host, pluginVersion: "6.0.0" };
...
async function resolveGrillForAnswer(ctx) {
  ...
  const next = await mutatePrepared(root2, featureId, expectedRevision, "decision-answered", async () => ({
    mutate: (draft) => {
      response = resolveResponseForAnswer(draft, interaction, { source: credential.source, action: credential.source === "elicitation" ? credential.action : void 0, comment: credential.source === "elicitation" ? credential.comment : void 0, userReply: credential.source === "text" ? credential.promptText : void 0, promptText, promptEventId, host });
      if (!response) throw new DevFlowError("INTERACTION_NOT_RESOLVED", interaction.id);
...
function featureDirectory2(root2, id) {
  return path20.join(root2, ".dev-flow", "features", id);
}
async function readEvidenceSnapshot(root2, id, snapshotPath) {
  const raw = await readFile17(path20.join(featureDirectory2(root2, id), snapshotPath), "utf8");
...
async function writeEvidenceSnapshot(root2, id, snapshot, fingerprint2, directory) {
  ...
  const file = path20.join(featureRoot, snapshotPath);
  await mkdir9(path20.dirname(file), { recursive: true });
  await writeFile2(file, JSON.stringify(snapshot));
  return snapshotPath;
}
...
async function baselineForRecord(root2, state, kind, recordId, baselineRef, aggregateFingerprint) {
  ...
  try {
    const manifestBytes = await readEvidenceObject(root2, state.featureId, ref);
    const manifest = parseEvidenceBaselineManifest(JSON.parse(manifestBytes.toString("utf8")));
    const snapshotBytes = await readEvidenceObject(root2, state.featureId, manifest.snapshotRef);
...
async function contentBoundBaselines(root2, state) {
  const records = [];
  const invalidatedAt = state.lastInvalidation?.at ? Date.parse(state.lastInvalidation.at) : Number.NaN;
  for (const claim of state.governance?.claims ?? []) {
    ...
    if (!live) continue;
    const record = await baselineForRecord(root2, state, "claim", claim.recordId, claim.baselineRef, basis);
  ...
  for (const authorization of state.governance?.authorizations ?? []) {
    ...
    const record = await baselineForRecord(root2, state, "authorization", authorization.recordId, authorization.baselineRef, basis);
    if (record) records.push(record);
  }
  ...
  return baselineForRecord(root2, state, "authorization", accepted.recordId, accepted.baselineRef, accepted.basis.sha256).then((record) => record ? [record] : []);
}
...
async function invalidateAffectedClaims(root2, id, expectedRevision) {
  ...
  const baselines = await contentBoundBaselines(root2, state);
  ...
  for (const baseline of baselines) {
    ...
    if (baseline.snapshotFiles) {
      afterFiles ??= (await snapshotGovernedRoots(root2, config)).filter((file) => state.workspace.ownership[file.path] !== "excluded");
...
function presentWorkspaceOwnership(state, paths, options = {}) {
  ...
  const presentationEventId = options.presentationEventId ?? randomUUID10();
  const basisHash2 = createHash20("sha256").update(JSON.stringify({ kind: "workspace-ownership", paths: batchPaths, fingerprint: state.workspace.lastWorkspaceFingerprint })).digest("hex");
  const interaction = createInteraction(state, {
    kind: "workspace-ownership",
    target: `workspace:${createHash20("sha256").update(batchPaths.join("\n")).digest("hex").slice(0, 16)}:${currentPaths[0] ?? "batch"}`,
    basisHash: basisHash2,
...
async function resolveOwnershipForAnswer(ctx) {
  ...
  const matchedId = credential.source === "elicitation" ? credential.action : matchDecisionReply(decision, promptText ?? credential.promptText).option.id;
...
function presentTaskSwitch(state, input) {
  const presentationEventId = randomUUID10();
  const interaction = createInteraction(state, {
    kind: "task-switch",
    target: `task-switch:${input.targetFeatureId}`,
    basisHash: createHash20("sha256").update(`${state.featureId}
...
async function reconcileWorkspace(root2, id, expectedRevision, host) {
  await invalidateAffectedClaims(root2, id, expectedRevision);
  const state = await readState(root2, id);
  const config = await readProjectConfig(root2);
  ...
  let presentationEventId;
  return mutate(root2, id, state.revision, "workspace-reconciled", (draft) => {
...
var init_ownership_workflow = __esm({
  "plugins/dev-flow/src/core/ownership-workflow.ts"() {
    ...
    TASK_SWITCH_QUESTION = "\u5F53\u524D\u5DF2\u6709\u4E00\u4E2A\u8FDB\u884C\u4E2D\u7684\u4EFB\u52A1\u3002\u5F00\u59CB\u65B0\u4EFB\u52A1\u524D\uFF0C\u4F60\u5E0C\u671B\u5982\u4F55\u5904\u7406\u65E7\u4EFB\u52A1\uFF1F";
  }
});
...
async function lockClassification(root2, id, expectedRevision, facts, boundaryAudit) {
  ...
  if (selected.classification.routeConfirmationRequired) {
    ...
    return mutatePrepared(root2, id, expectedRevision, "route-confirmation-presented", async () => ({ mutate: (draft) => {
      ...
      presentationEventId = randomUUID11();
...
function idFor(question, refs = []) {
  return `DEC-${createHash22("sha256").update(`${question}
...
function presentationEventIndex(events, input) {
  ...
  if (input.presentationEventSequence !== void 0) {
    const cursor = input.presentationEventSequence;
    const firstAtOrAfterCursor = events.findIndex((record) => record.eventSequence !== void 0 && record.eventSequence >= cursor);
    if (firstAtOrAfterCursor < 0) return events.length > 0 ? events.length - 1 : void 0;
    return Math.max(-1, firstAtOrAfterCursor - 1);
  }
  const index = events.findIndex((record) => record.revision >= input.presentedRevision && Date.parse(record.at) >= Date.parse(input.presentedAt));
...
function latestUnconsumedPromptEvent(events, state, interaction, host) {
  ...
  const presentationIndex = presentationEventIndex(events, {
    ...
    presentedRevision,
    ...interaction.presentationEventId ? { presentationEventId: interaction.presentationEventId } : {},
    ...interaction.presentationEventSequence !== void 0 ? { presentationEventSequence: interaction.presentationEventSequence } : {}
  });
  ...
  const candidates = events.flatMap((record, index) => {
    ...
    if (presentationIndex !== void 0 && index <= presentationIndex) return [];
    if (presentationIndex === void 0 && (record.revision <= presentedRevision || Date.parse(prompt.at) < Date.parse(interaction.presentedAt))) return [];
...
async function recordDecision(root2, id, expectedRevision, question, evidence, conclusion, factRefs = [], host) {
  ...
  const state = await mutate(root2, id, expectedRevision, "decision-ratification-presented", (draft) => {
    interaction = createInteraction(draft, {
      ...
      target,
      basisHash: createHash23("sha256").update(`${decision.id}
${evidence.trim()}
...
function revisionSuccessorId(question, newConclusion, reason) {
  return `DEC-${createHash23("sha256").update(`${question}
${newConclusion.trim()}
...
async function reviseDecision(root2, id, expectedRevision, decisionId, newConclusion, reason, host) {
  ...
  const state = await mutate(root2, id, expectedRevision, "decision-revision-presented", (draft) => {
    ...
    interaction = createInteraction(draft, {
      ...
      target,
      basisHash: createHash23("sha256").update(`${decisionId}
${newConclusion.trim()}
...
async function resolveRatificationForAnswer(ctx) {
  ...
  const pending = pendingDecisionForState(state);
  const matchedId = credential.source === "elicitation" ? credential.action : matchDecisionReply(pending, promptText ?? credential.promptText).option.id;
...
function diagnostic(artifactKind, position, message, line, field) {
  return { code: "TRACE_MARKDOWN_INVALID", artifactKind, position, message, ...line === void 0 ? {} : { line }, ...field === void 0 ? {} : { field } };
}
function nfc(value) {
  return value.normalize("NFC").trim();
}
function parseFieldValue(result, block, field) {
  ...
  const items = inner.length === 0 ? [] : inner.split(",").map((item) => item.normalize("NFC").trim()).filter((item) => item.length > 0);
...
function buildNode(result, block, fields) {
  const scalar = (key) => {
  ...
  switch (block.kind) {
    case "requirement":
      return { kind: "requirement", id: block.id };
    case "acceptance-criterion": {
      ...
      if (kind !== void 0 && !["behavior-test", "type-check", "rule-check", "file-check", "human-acceptance"].includes(kind)) {
      ...
      if (kind === void 0 && (reason !== void 0 || target !== void 0)) {
        result.diagnostics.push(diagnostic(result.artifactKind, block.id, "verification_reason/target \u5FC5\u987B\u4E0E verification_kind \u4E00\u8D77\u63D0\u4F9B", fields.get(reason !== void 0 ? "verification_reason" : "verification_target")?.line));
        return void 0;
      }
      const disposition = kind === void 0 ? void 0 : {
        kind,
    ...
    case "task": {
      ...
      if (tdd !== void 0 && tdd !== "test-first" && tdd !== "direct") {
    ...
    case "implementation-unit":
      return {
        ...
        fileScope: list("file_scope"),
        covers: list("covers"),
        forwardVerification: list("forward_verification")
      };
    case "recovery":
      return {
        kind: "recovery",
        id: block.id,
        stepRef: scalar("step_ref"),
        recoveryKind: scalar("recovery_kind"),
        method: scalar("method"),
        riskRef: scalar("risk_ref")
      };
  }
}
function parseTraceMarkdown(markdown, artifactKind) {
  const result = { artifactKind, ok: true, diagnostics: [], blocks: [], nodes: [] };
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    anchors.push({ id, kind, line: index, comment });
  }
  for (let anchorIndex = 0; anchorIndex < anchors.length; anchorIndex += 1) {
    const anchor = anchors[anchorIndex];
    const endLine = anchors[anchorIndex + 1]?.line ?? lines.length;
    const block = { id: anchor.id, kind: anchor.kind, anchorLine: anchor.line + 1, fields: [] };
    ...
    if (node !== void 0) result.nodes.push(node);
  }
...
var allowedKindsByArtifact, allowedFieldsByKind, requiredFieldsByKind, listFields, anchorKindPrefix, anchorPattern, fieldPattern;
var init_traceability_markdown = __esm({
  "plugins/dev-flow/src/core/traceability-markdown.ts"() {
    "use strict";
    init_traceability();
    allowedKindsByArtifact = {
      requirements: /* @__PURE__ */ new Set(["requirement", "acceptance-criterion"]),
      "implementation-plan": /* @__PURE__ */ new Set(["task", "test", "implementation-unit", "recovery"])
    };
    allowedFieldsByKind = {
      requirement: /* @__PURE__ */ new Set([]),
      "acceptance-criterion": /* @__PURE__ */ new Set(["parent_requirement", "verification_kind", "verification_reason", "verification_target"]),
      task: /* @__PURE__ */ new Set(["covers", "implementation_unit", "tdd"]),
      test: /* @__PURE__ */ new Set(["verifies"]),
      "implementation-unit": /* @__PURE__ */ new Set(["tasks", "depends_on", "file_scope", "covers", "forward_verification"]),
      recovery: /* @__PURE__ */ new Set(["step_ref", "recovery_kind", "method", "risk_ref"])
    };
    ...
    listFields = /* @__PURE__ */ new Set(["covers", "tasks", "depends_on", "file_scope", "verifies", "forward_verification"]);
    anchorKindPrefix = {
      REQ: "requirement",
      AC: "acceptance-criterion",
      TASK: "task",
    ...
    anchorPattern = /^<!--\s*dev-flow:id=(REQ|AC|TASK|TEST|UNIT|REC)-([0-9]{3,})\s+kind=(requirement|acceptance-criterion|task|test|implementation-unit|recovery)\s*-->\s*$/;
    fieldPattern = /^- ([a-z][a-z0-9_]*):[ \t]*(.*)$/;
  }
});
...
function markdownDiagnostics(parsed) {
...
async function readArtifactContents(root2, id, state, artifactKind) {
  const artifact = state.artifacts[artifactKind];
  if (!artifact) throw new DevFlowError("MISSING_REQUIRED_ARTIFACT", artifactKind);
  return {
    artifact,
    contents: await readFile18(path21.join(featureDirectory3(root2, id), normalizeUnicode(artifact.path)), "utf8")
  };
}
async function compileArtifactPlanFromMarkdown(root2, id, state, options) {
  const { artifact, contents } = await readArtifactContents(root2, id, state, options.artifactKind);
  const { config, sha256: projectConfigSha256 } = await readProjectConfigSnapshot(root2);
  const parsed = parseTraceMarkdown(contents, options.artifactKind);
  const nodes = parsed.nodes;
  if (!parsed.ok) {
    const diagnostics2 = markdownDiagnostics(parsed);
    ...
    return { result: { ok: false, diagnostics: diagnostics2 }, artifact, config, nodes };
  }
  const semanticSha256 = createHash24("sha256").update(stableJson({ nodes })).digest("hex");
  const currentLedger = await readTraceabilityForArtifactReplacement(root2, state, options.artifactKind);
  const input = {
    route: state.route,
    artifactKind: options.artifactKind,
    artifactSha256: createHash24("sha256").update(contents).digest("hex"),
    ...
    verificationCommandHashes: verificationCommandHashes(config),
    verificationCommandGuarantees: Object.fromEntries(config.verification.commands.map((command2) => [command2.id, [...command2.provides]])),
    nextStateRevision: options.nextStateRevision,
    riskLabels: state.classification.riskLabels
  };
  return { input, result: compilePlan(input), artifact, config, semanticSha256, nodes };
}
var featureDirectory3;
var init_plan_compile_context = __esm({
  "plugins/dev-flow/src/core/plan-compile-context.ts"() {
    ...
    init_traceability_store();
    featureDirectory3 = (root2, id) => path21.join(root2, ".dev-flow", "features", id);
  }
});
...
function shaOrNone(value) {
  return typeof value === "string" && /^[a-f0-9]{64}$/.test(value) ? value : "0".repeat(64);
}
function requirementsBasis(state, ledger) {
  const artifactSha256 = shaOrNone(state.artifacts.requirements?.sha256);
  const nodes = Object.values(ledger.nodes).filter((node) => node.status !== "tombstoned" && node.sourceArtifact === "requirements").sort((left, right) => left.id.localeCompare(right.id)).map(({ sourceArtifact: _sourceArtifact, sourceSha256: _sourceSha256, sourceAnchor: _sourceAnchor, sourceBlockSha256: _sourceBlockSha256, status: _status, ...semantic }) => semantic);
  const sliceSha256 = hash(stableJson({ nodes }));
  return { artifactSha256, semanticSha256: sliceSha256, sliceSha256 };
}
function executionSemanticBasisHashForLedger(ledger, config) {
  const executionNodes = Object.values(ledger.nodes).filter((node) => node.status === "current" && node.kind !== "test").map((node) => node.kind === "implementation-unit" ? {
    kind: node.kind,
    id: node.id,
    tasks: node.tasks,
    dependsOn: node.dependsOn,
  ...
  const executionVerificationRefs = Object.values(ledger.nodes).filter((node) => node.status === "current" && node.kind === "implementation-unit").flatMap((node) => node.forwardVerification);
  return hash(JSON.stringify({
    nodes: executionNodes,
    verificationCommandHashes: verificationCommandHashesForRefs(config, executionVerificationRefs)
  }));
}
function projectionForCompilation(compilation) {
  return {
...
async function revisePlanFromMarkdown(root2, id, expectedRevision, host) {
  const initial = await readState(root2, id);
  if (initial.revision !== expectedRevision) throw new DevFlowError("STATE_REVISION_CONFLICT", "state revision changed", { currentRevision: initial.revision });
  ...
  const compilation = await compileArtifactPlanFromMarkdown(root2, id, initial, {
  ...
  const traceStored = await putEvidenceObject(root2, id, "trace", Buffer.from(canonicalTraceJson(newLedger), "utf8"));
  const proposal = {
    schemaVersion: 1,
    featureId: id,
  ...
  const proposalStored = await putEvidenceObject(root2, id, "artifact-proposal", Buffer.from(`${stableJson(proposal)}
`, "utf8"));
...
async function prepareProposalApplication(root2, featureId, current, proposalRef, nextStateRevision) {
  if (!proposalRef) throw new DevFlowError("PLAN_REVISION_PROPOSAL_MISSING", "plan revision proposal ref is missing");
  const proposalBytes = await readEvidenceObject(root2, featureId, proposalRef);
  const proposal = parsePlanRevisionProposal(JSON.parse(proposalBytes.toString("utf8")));
  if (proposal.featureId !== featureId) throw new DevFlowError("PLAN_REVISION_STALE", "proposal featureId mismatch");
  const compilation = await compileArtifactPlanFromMarkdown(root2, featureId, current, {
    artifactKind: "implementation-plan",
    nextStateRevision
  });
  if (!compilation.result.ok || !compilation.result.ledger || !compilation.input) {
  ...
  const configSha = (await readProjectConfigSnapshot(root2)).sha256;
  ...
  const expectedTrace = await readEvidenceObject(root2, featureId, proposal.compiledTrace);
  const storedLedger = JSON.parse(expectedTrace.toString("utf8"));
  const currentLedger = JSON.parse(canonicalTraceJson(proposalLedger));
  ...
  const tracePointer = await writeTraceSnapshot(root2, proposalLedger);
  const reviewPointer = await prepareReviewInvalidation(root2, current, nextStateRevision);
  return {
  ...
  const next = await mutatePrepared(root2, featureId, expectedRevision, confirms ? "plan-revised" : "plan-revision-cancelled", async (current, nextStateRevision) => {
    if (confirms) {
      ...
      if (live?.planRevisionProposal) {
        proposalApplication = await prepareProposalApplication(root2, featureId, current, live.planRevisionProposal, nextStateRevision);
      } else {
        ...
        currentArtifactSha256 = createHash25("sha256").update(contents).digest("hex");
        ...
        if (confirms) {
          ...
          if (sideEffects.length) {
            createInteraction(draft, {
              ...
              basisHash: createHash25("sha256").update(`${featureId}
...
async function resolveApprovalForAnswer(ctx) {
  ...
  if (credential.source === "text") {
    ...
    promptText = resolvedProvenance.promptEventId ? events.find((item) => item.type === "host-event" && item.data.eventId === resolvedProvenance.promptEventId)?.data?.text : void 0;
  }
  let response;
  const next = await mutatePrepared(root2, featureId, expectedRevision, "approval-interaction-resolved", async (current) => {
    ...
    const basisHash2 = digest7(approvalBasis(current, approval));
    ...
    return {
      mutate: (draft) => {
        const phraseText = promptText ?? (credential.source === "text" ? credential.promptText : void 0);
        response = resolveResponseForAnswer(draft, interaction, {
          source: credential.source,
          action: credential.source === "elicitation" ? credential.action : void 0,
          comment: credential.source === "elicitation" ? credential.comment : void 0,
...
var init_approval_interactions = __esm({
  "plugins/dev-flow/src/core/approval-interactions.ts"() {
    ...
    init_obligations();
    init_user_interactions();
    digest7 = (value) => createHash26("sha256").update(JSON.stringify(value)).digest("hex");
  }
});

// plugins/dev-flow/src/core/acceptance-store.ts
import { createHash as createHash27 } from "node:crypto";
import { copyFile as copyFile2, lstat as lstat6, mkdir as mkdir10, readFile as readFile19 } from "node:fs/promises";
...
async function storeScreenshotArtifact(root2, featureId, sourcePath) {
  ...
  const sourceAbsolute = path22.join(root2, source);
  let metadata;
  try {
    metadata = await lstat6(sourceAbsolute);
  } catch {
  ...
  let bytes;
  try {
    bytes = await readFile19(sourceAbsolute);
  } catch {
    throw new DevFlowError("INVALID_ACCEPTANCE_EVIDENCE", "\u622A\u56FE\u6587\u4EF6\u4E0D\u5B58\u5728\u6216\u4E0D\u53EF\u8BFB\u53D6\u3002", { path: source });
  }
  if (!imageValid(bytes)) {
  ...
  const artifactSha256 = digest8(bytes);
  const ext = path22.extname(source).toLowerCase() || ".bin";
  const artifactPath = `acceptance/${artifactSha256}${ext}`;
  const featureRoot = path22.join(root2, ".dev-flow", "features", featureId);
  await mkdir10(path22.join(featureRoot, "acceptance"), { recursive: true });
  await copyFile2(sourceAbsolute, path22.join(featureRoot, artifactPath));
...
var init_acceptance_store = __esm({
  "plugins/dev-flow/src/core/acceptance-store.ts"() {
    "use strict";
    init_errors();
    digest8 = (value) => createHash27("sha256").update(value).digest("hex");
  }
});
...
async function recordAcceptanceEvidence(root2, id, expectedRevision, input) {
  ...
  const evidenceId = `AC-EVIDENCE-${randomUUID12()}`;
  return mutate(root2, id, expectedRevision, "acceptance-evidence-recorded", (state) => {
    ...
    state.lastUpdatedBy = { host: input.host, pluginVersion: "6.0.0" };
  });
}
function dispositionHash(state, criterionIds, fingerprint2) {
  ...
  return digest9(JSON.stringify({ criterionIds, fingerprint: fingerprint2, entries }));
}
async function presentAcceptanceConfirmation(root2, id, expectedRevision, acceptanceCriterionIds) {
  ...
  const hash3 = dispositionHash(initial, selected, fingerprint2);
  let created;
  const state = await mutate(root2, id, expectedRevision, "acceptance-confirmation-presented", (draft) => {
    ...
    if (currentHash !== hash3) throw new DevFlowError("ACCEPTANCE_CONFIRMATION_STALE", "\u9A8C\u6536\u4F9D\u636E\u5DF2\u53D8\u5316\uFF0C\u8BF7\u91CD\u65B0\u5448\u73B0\u3002", { retryOriginal: true });
    created = createInteraction(draft, {
      kind: "acceptance-confirmation",
      target: `acceptance-confirmation:${selected.join(",")}`,
      basisHash: digest9(JSON.stringify({ fingerprint: fingerprint2, hash: hash3, selected })),
      question: `\u8BF7\u786E\u8BA4\u4EE5\u4E0B\u9A8C\u6536\u6761\u4EF6\u5728\u5F53\u524D\u4EA4\u4ED8\u5185\u5BB9\u4E0A\u5DF2\u7ECF\u8FBE\u5230\u9884\u671F\uFF1A${selected.join("\u3001")}`,
      options: [{ id: "confirm", label: "\u786E\u8BA4\u9A8C\u6536" }, { id: "decline", label: "\u6682\u4E0D\u786E\u8BA4" }],
      acceptanceConfirmation: { acceptanceCriterionIds: selected, deliveryFingerprint: fingerprint2, dispositionHash: hash3 }
    });
  });
  ...
  const next = await mutatePrepared(root2, featureId, expectedRevision, "acceptance-confirmation-resolved", async (current) => {
    ...
    return {
      mutate: (draft) => {
        ...
        const credentialId = `CRED-ACCEPTANCE-${randomUUID12()}`;
...
var init_acceptance = __esm({
  "plugins/dev-flow/src/core/acceptance.ts"() {
    ...
    init_errors();
    init_user_interactions();
    digest9 = (value) => createHash28("sha256").update(value).digest("hex");
  }
});
...
async function writeVerificationOutput(root2, featureId, outputPath, output) {
  const file = path23.join(root2, ".dev-flow", "features", featureId, outputPath);
  await mkdir11(path23.dirname(file), { recursive: true });
  await writeFile3(file, output);
}
var run4;
var init_verification_store = __esm({
  "plugins/dev-flow/src/core/verification-store.ts"() {
    "use strict";
    run4 = promisify5(execFile5);
  }
});
...
function verificationCommandSliceStale(state, config) {
  ...
  return Object.entries(attempt.verificationCommandHashes).some(([id, hash3]) => current[id] !== hash3);
}
...
async function runVerification(root2, id, expectedRevision, host, commandIds) {
  ...
  return mutate(root2, id, expectedRevision, "verification-recorded", async (state) => {
    ...
    if (exitCode === 0 && acceptance.complete) {
      ...
      const claimId = `CLAIM-${createHash29("sha256").update(`verification-current|${fingerprint2}`).digest("hex").slice(0, 16)}`;
      const claims = [...gov.claims];
      if (!claims.some((claim) => claim.recordId === claimId)) {
        const baselineRef = await captureEvidenceBaseline(root2, state, config, {
          kind: "verification-current",
          target: id,
          recordId: claimId,
          at: finishedAt
        }).then((captured) => captured.ref).catch(() => void 0);
        claims.push({
          recordId: claimId,
          kind: "claim",
      ...
      const signature = `${exitReason}:${createHash29("sha256").update(fullOutput).digest("hex").slice(0, 16)}`;
      state.repair = recordRepairAttempt(state.repair ?? startRepairLoop(), signature, output.slice(-3));
    }
    state.lastUpdatedBy = { host, pluginVersion: "6.0.0" };
  });
}
...
var DEFAULT_COMMAND_TIMEOUT_MS, DEFAULT_COMMAND_MAX_OUTPUT_BYTES;
var init_verification = __esm({
  "plugins/dev-flow/src/core/verification.ts"() {
    ...
    DEFAULT_COMMAND_TIMEOUT_MS = 12e4;
    DEFAULT_COMMAND_MAX_OUTPUT_BYTES = 1024 * 1024;
  }
});

// plugins/dev-flow/src/core/checkpoints.ts
import { randomUUID as randomUUID13, createHash as createHash30 } from "node:crypto";
import { access as access2, mkdir as mkdir12, open as open9, readFile as readFile20, readlink as readlink4, readdir as readdir7, rename as rename8 } from "node:fs/promises";
...
async function writeAtomic(file, contents) {
  ...
  const directory = await open9(path24.dirname(file), "r");
  try {
...
async function packCheckpointBlobRefs(root2, featureId, records) {
  const refs = {};
  for (const record of records) {
    for (const blobSha256 of [record.beforeBlobSha256, record.afterBlobSha256]) {
      if (blobSha256 === void 0 || refs[blobSha256]) continue;
      const file = path24.join(featureDirectory4(root2, featureId), blobPath(blobSha256));
      const bytes = await readFile20(file);
      if (digest10(bytes) !== blobSha256) {
        throw new DevFlowError("CHECKPOINT_HASH_MISMATCH", "\u6355\u83B7 checkpoint blob \u65F6 governed \u6587\u4EF6\u53D1\u751F\u53D8\u5316\u3002", { blobSha256 });
      }
      const stored = await putEvidenceObject(root2, featureId, "checkpoint-pack", bytes);
...
async function writeBlobIfAbsent(root2, featureId, bytes) {
  const sha2564 = digest10(bytes);
  const file = path24.join(featureDirectory4(root2, featureId), blobPath(sha2564));
  if (await pathExists2(file)) return sha2564;
  await mkdir12(path24.dirname(file), { recursive: true });
...
async function captureUnitBaseline(root2, featureId, unitId, snapshot) {
  for (const file2 of snapshot) {
    const bytes = file2.kind === "symlink" ? Buffer.from(await readlink4(path24.join(root2, file2.path))) : await readFile20(path24.join(root2, file2.path));
    if (digest10(bytes) !== file2.sha256) {
  ...
  const file = path24.join(featureDirectory4(root2, featureId), baselinePath(unitId));
  await mkdir12(path24.dirname(file), { recursive: true });
...
async function readCheckpointBaseline(root2, featureId, unitId) {
  ...
  let raw;
  try {
    raw = await readFile20(file, "utf8");
  } catch {
...
function diffSnapshots(before, after) {
  ...
  for (const [hash3, deletedFiles] of deletedByHash) {
    const addedFiles = addedByHash.get(hash3) ?? [];
    if (deletedFiles.length === 1 && addedFiles.length === 1) {
      ...
      records.push({
        ...
        beforeSha256: hash3,
        afterSha256: hash3,
        beforeBlobSha256: hash3,
        afterBlobSha256: hash3,
...
async function nextCheckpointSequence(root2, featureId) {
  const directory = path24.join(featureDirectory4(root2, featureId), "checkpoints", "manifests");
  let entries;
  try {
    entries = await readdir7(directory);
  } catch {
    return 1;
  }
  let max = 0;
...
async function checkpointImplementationUnit(root2, id, expectedRevision, unitId, options = {}) {
  ...
  const featureDir = featureDirectory4(root2, id);
  const manifestsDir = path24.join(featureDir, "checkpoints", "manifests");
  let orphan;
  let entries;
  try {
    entries = await readdir7(manifestsDir);
  } catch (error) {
  ...
  for (const entry of entries.sort().reverse()) {
    ...
    let candidate;
    try {
      candidate = parseCheckpointManifest(JSON.parse(await readFile20(path24.join(manifestsDir, entry), "utf8")));
    } catch (error) {
  ...
  const manifestFile = path24.join(featureDir, manifestPath(checkpointId));
  const attempts = [];
  ...
  for (const record of records) {
    ...
    const bytes = record.afterKind === "symlink" ? Buffer.from(await readlink4(path24.join(root2, record.path))) : await readFile20(path24.join(root2, record.path));
    if (digest10(bytes) !== record.afterSha256) {
  ...
  const blobRefs = await packCheckpointBlobRefs(root2, id, records);
  const manifest = {
    schemaVersion: 3,
    checkpointId,
    unitId: implementationUnitId,
    ...
    completedAt: (/* @__PURE__ */ new Date()).toISOString(),
    files: records,
    forwardPatchSha256: digest10(forwardPatch),
    reversePatchSha256: digest10(reversePatch),
    ...
    verificationCommandHashes: Object.fromEntries([...preflightCommands, ...commands].map((command2) => [command2.id, currentCommandHashes[command2.id] ?? digest10(JSON.stringify(command2))])),
    blobRefs
  };
  const validated = parseCheckpointManifest(JSON.parse(JSON.stringify(manifest)));
  await mkdir12(path24.join(featureDir, "checkpoints", "patches"), { recursive: true });
  await mkdir12(path24.dirname(manifestFile), { recursive: true });
  await writeAtomic(path24.join(featureDir, "checkpoints", "patches", `${manifest.forwardPatchSha256}.json`), forwardPatch);
  await writeAtomic(path24.join(featureDir, "checkpoints", "patches", `${manifest.reversePatchSha256}.json`), reversePatch);
  const manifestContents = `${JSON.stringify(validated, null, 2)}
`;
  const temp = `${manifestFile}.${randomUUID13()}.tmp`;
  const handle = await open9(temp, "w");
  try {
  ...
  await rename8(temp, manifestFile);
  const manifestDir = await open9(path24.dirname(manifestFile), "r");
  try {
...
var digest10, featureDirectory4;
var init_checkpoints = __esm({
  "plugins/dev-flow/src/core/checkpoints.ts"() {
    ...
    init_change_invalidation();
    init_evidence_store2();
    digest10 = (value) => createHash30("sha256").update(value).digest("hex");
    featureDirectory4 = (root2, featureId) => path24.join(root2, ".dev-flow", "features", featureId);
  }
});
...
function implementationUnitBasisHash(state) {
  return digest11(canonicalReviewValueJson({
...
async function beginImplementationUnit(root2, id, expectedRevision, unitId) {
  return mutate(root2, id, expectedRevision, "implementation-unit-begun", async (state) => {
    ...
    target.beginNonce = randomUUID14();
...
var init_implementation_units = __esm({
  "plugins/dev-flow/src/core/implementation-units.ts"() {
    ...
    init_approval_basis();
    init_plan_graph();
    digest11 = (value) => createHash31("sha256").update(value).digest("hex");
  }
});

// plugins/dev-flow/src/core/rollback-journal.ts
import { randomUUID as randomUUID15 } from "node:crypto";
import { mkdir as mkdir13, readFile as readFile21, rm, rmdir } from "node:fs/promises";
import { hostname } from "node:os";
import path25 from "node:path";
function isSha2566(value) {
  return typeof value === "string" && /^[a-f0-9]{64}$/.test(value);
}
function validateRollbackTransaction(value) {
  ...
  const validPlan = Array.isArray(transaction?.filePlan) && transaction.filePlan.every((action) => {
    ...
    if (candidate.action === "restore" && (!isSha2566(candidate.blobSha256) || typeof candidate.mode !== "string" || !/^[0-7]{3,4}$/.test(candidate.mode))) return false;
    if (candidate.blobSha256 !== void 0 && !isSha2566(candidate.blobSha256)) return false;
...
async function readRollbackTransaction(root2, featureId) {
  let raw;
  try {
    raw = await readFile21(rollbackTxnPath(root2, featureId), "utf8");
  } catch (error) {
...
function driveLeasePath(root2, featureId, transactionId) {
  return path25.join(features(root2), featureId, "checkpoints", "recovery", `${transactionId}-drive-lease.json`);
}
...
async function readLeaseAt(leaseFile, transactionId) {
  try {
    const raw = await readFile21(leaseFile, "utf8");
...
async function writeDriveLeasePair(root2, featureId, transactionId, lease) {
  ...
  await mkdir13(path25.dirname(legacyFile), { recursive: true });
  await mkdir13(path25.dirname(sidecarFile), { recursive: true });
...
async function releaseRollbackDriveLease(root2, featureId, lease) {
  ...
  try {
    ...
    let sidecar;
    try {
      sidecar = JSON.parse(await readFile21(sidecarFile, "utf8"));
    } catch {
    }
    ...
    try {
      const legacyExisting = JSON.parse(await readFile21(legacyFile, "utf8"));
    ...
    try {
      await rmdir(path25.dirname(legacyFile));
    } catch {
    }
  } finally {
    await release();
...
var features, rollbackTxnPath, rollbackTransactionPhases, ROLLBACK_DRIVE_LEASE_STALE_MS, ROLLBACK_DRIVE_LEASE_HEARTBEAT_MS;
var init_rollback_journal = __esm({
  "plugins/dev-flow/src/core/rollback-journal.ts"() {
    "use strict";
    init_errors();
    init_state_store();
    features = (root2) => path25.join(root2, ".dev-flow", "features");
    rollbackTxnPath = (root2, featureId) => path25.join(features(root2), featureId, "rollback-transaction.json");
    rollbackTransactionPhases = /* @__PURE__ */ new Set(["prepared", "backing-up", "rolling-back", "verifying", "committed", "compensating", "compensated"]);
    ROLLBACK_DRIVE_LEASE_STALE_MS = 3e4;
    ROLLBACK_DRIVE_LEASE_HEARTBEAT_MS = 1e4;
  }
});

// plugins/dev-flow/src/core/rollback.ts
import { createHash as createHash32, randomUUID as randomUUID16 } from "node:crypto";
import { access as access3, chmod, lstat as lstat7, mkdir as mkdir14, open as open10, readFile as readFile22, readlink as readlink5, rename as rename9, rm as rm2, symlink } from "node:fs/promises";
...
async function previewRollback(root2, featureId, targetCheckpointId) {
  ...
  const stale = suffix.filter((manifest) => manifest.verificationCommandHashes ? Object.entries(manifest.verificationCommandHashes).some(([id, hash3]) => verificationCommandHashes2[id] !== hash3) : manifest.projectConfigSha256 !== projectConfigSha256);
  if (stale.length) {
  ...
  const planAction = (path32, action) => {
    filePlan.set(path32, action);
  };
  for (const manifest of undoManifests) {
    for (const record of manifest.files) {
      switch (record.change) {
  ...
  const previewBasisHash = digest12(canonicalReviewValueJson({
    targetCheckpointId,
...
async function fsyncDirectory6(directory) {
  const handle = await open10(directory, "r");
  try {
    await handle.sync();
  } finally {
    await handle.close();
...
async function writeFileAtomicMode(file, bytes, mode) {
  await mkdir14(path26.dirname(file), { recursive: true });
  ...
  await fsyncDirectory6(path26.dirname(file));
}
...
async function readBackupManifest(manifestFile, transactionId) {
  let raw;
  try {
    raw = await readFile22(manifestFile, "utf8");
  } catch {
...
async function captureBackup(root2, featureId, journal, config, options) {
  const dir = path26.join(featureDirectory5(root2, featureId), journal.backupDirectory);
  const manifestFile = path26.join(dir, "backup-manifest.json");
  ...
  await mkdir14(path26.join(dir, "files"), { recursive: true });
  await mkdir14(path26.join(dir, "trash"), { recursive: true });
  const snapshot = await snapshotGovernedRoots(root2, config);
  let first = true;
  for (const file of snapshot) {
    const bytes = file.kind === "symlink" ? Buffer.from(await readlink5(path26.join(root2, file.path))) : await readFile22(path26.join(root2, file.path));
    if (digest12(bytes) !== file.sha256) {
      throw new DevFlowError("ROLLBACK_HASH_MISMATCH", "protected files changed while capturing the rollback backup", { path: file.path });
    }
    const blobFile = path26.join(dir, "files", file.sha256);
...
async function assertPathMatchesBackupExpectation(root2, filePath, expected) {
  const absolute = path26.join(root2, filePath);
  if (expected) {
    let metadata;
    let bytes;
    try {
      metadata = await lstat7(absolute);
      bytes = expected.kind === "symlink" ? Buffer.from(await readlink5(absolute)) : await readFile22(absolute);
    } catch {
    ...
    if (digest12(bytes) !== expected.sha256 || mode !== expected.mode || kind !== (expected.kind ?? "file")) {
      throw new DevFlowError("ROLLBACK_HASH_MISMATCH", "workspace drifted from the pre-rollback backup before a file action", {
        path: filePath,
        expected: "present",
        actual: "changed",
...
async function applyFilePlan(root2, featureId, journal, options) {
  const dir = path26.join(featureDirectory5(root2, featureId), journal.backupDirectory);
  const trash = path26.join(dir, "trash");
  const backup = await readBackupManifest(path26.join(dir, "backup-manifest.json"), journal.transactionId);
  ...
  for (let index = journal.nextFileIndex; index < journal.filePlan.length; index += 1) {
    ...
    const target = path26.join(root2, action.path);
    if (action.action === "restore") {
      const blobFile = path26.join(featureDirectory5(root2, featureId), blobPath(action.blobSha256));
      let bytes;
      try {
        bytes = await readFile22(blobFile);
      } catch {
      ...
      if (digest12(bytes) !== action.blobSha256) {
      ...
      const trashFile = path26.join(trash, `${String(index).padStart(4, "0")}-${path26.basename(action.path)}`);
      if (await pathExists3(target)) {
        await mkdir14(trash, { recursive: true });
        await rename9(target, trashFile);
        await fsyncDirectory6(path26.dirname(target));
        await fsyncDirectory6(trash);
    ...
    for (const action2 of journal.filePlan.slice(0, journal.nextFileIndex)) {
      ...
      if (action2.action === "delete") {
        if (await pathExists3(path26.join(root2, action2.path))) {
        ...
        let bytes;
        try {
          metadata = await lstat7(path26.join(root2, action2.path));
          bytes = expected.kind === "symlink" ? Buffer.from(await readlink5(path26.join(root2, action2.path))) : await readFile22(path26.join(root2, action2.path));
        } catch {
          throw new DevFlowError("ROLLBACK_HASH_MISMATCH", "workspace drifted after a rollback file action", {
            path: action2.path,
            source: "post-plan",
        ...
        const mode = (metadata.mode & 511).toString(8).padStart(3, "0");
        if (digest12(bytes) !== expected.sha256 || mode !== expected.mode) {
          throw new DevFlowError("ROLLBACK_HASH_MISMATCH", "workspace drifted after a rollback file action", {
            path: action2.path,
            source: "post-plan",
            expected: "present",
...
async function expectedPlanStateAfter(root2, featureId, journal, appliedCount) {
  const manifest = await readBackupManifest(
    path26.join(featureDirectory5(root2, featureId), journal.backupDirectory, "backup-manifest.json"),
...
async function compensateRollback(root2, featureId, journal, config, options) {
  ...
  const startedAt = (/* @__PURE__ */ new Date()).toISOString();
  try {
    const manifest = await readBackupManifest(path26.join(dir, "backup-manifest.json"), journal.transactionId);
    let restored = 0;
    for (const file of manifest.files) {
      ...
      if (file.kind === "symlink") await writeSymlinkAtomic(path26.join(root2, file.path), bytes.toString("utf8"));
      else await writeFileAtomicMode(path26.join(root2, file.path), bytes, file.mode);
      restored += 1;
    ...
    for (const file of current) {
      ...
      const trashFile = path26.join(trash, `extra-${digest12(file.path).slice(0, 16)}-${path26.basename(file.path)}`);
      await mkdir14(trash, { recursive: true });
      await rename9(path26.join(root2, file.path), trashFile);
      await fsyncDirectory6(path26.dirname(path26.join(root2, file.path)));
    }
    const after = await snapshotGovernedRoots(root2, config);
...
async function cleanupRollbackBackup(root2, featureId, journal) {
  const directory = path26.join(featureDirectory5(root2, featureId), journal.backupDirectory);
  await rm2(path26.join(directory, "files"), { recursive: true, force: true });
  await rm2(path26.join(directory, "trash"), { recursive: true, force: true });
  await rm2(path26.join(directory, "backup-manifest.json"), { force: true });
}
...
async function driveRollbackTransaction(root2, featureId, journal, options) {
  ...
  try {
    ...
    try {
      ...
      const commandSliceStale = journal.verificationCommandHashes ? Object.entries(journal.verificationCommandHashes).some(([id, hash3]) => commandHashes?.[id] !== hash3) : projectConfigSha256 !== journal.projectConfigSha256;
...
async function executeRollback(root2, featureId, expectedRevision, targetCheckpointId, options = {}) {
  ...
  const open12 = await readRollbackTransaction(root2, featureId);
  if (open12 && !rollbackTransactionFinished(open12)) {
    if (open12.targetCheckpointId !== targetCheckpointId) {
      throw new DevFlowError("ROLLBACK_TRANSACTION_MISMATCH", "an open rollback transaction targets a different checkpoint", {
        transactionId: open12.transactionId,
        openTargetCheckpointId: open12.targetCheckpointId,
        targetCheckpointId,
        recoveryHint: "Resume the open transaction with its original target checkpoint"
      });
    }
    return driveRollbackTransaction(root2, featureId, open12, options);
  }
  if (!rollbackExecutionAllowed(initial.route, initial.classification.controls)) {
    throw new DevFlowError("ROLLBACK_EXECUTION_NOT_ALLOWED", "\u5F53\u524D\u52A8\u6001\u8DEF\u7EBF\u6CA1\u6709\u542F\u7528 executable-rollback \u4E0E unit-chain \u63A7\u5236\u3002");
  }
  ...
  const transactionId = randomUUID16();
  const journal = {
    schemaVersion: 1,
    transactionId,
    featureId,
...
var digest12, featureDirectory5;
var init_rollback2 = __esm({
  "plugins/dev-flow/src/core/rollback.ts"() {
    ...
    init_approval_basis();
    init_user_interactions();
    digest12 = (value) => createHash32("sha256").update(value).digest("hex");
    featureDirectory5 = (root2, featureId) => path26.join(root2, ".dev-flow", "features", featureId);
  }
});
...
async function answer(input) {
  ...
  let resolvedCredential = credential.source === "elicitation" ? credential : {
    source: "text",
    promptEventId: credential.promptEventId ?? "",
    promptText: credential.promptText ?? "",
  ...
  if (credential.source === "text") {
    if (credential.promptEventId === void 0 || credential.promptText === void 0 || credential.promptAt === void 0) {
      const events = await readFeatureEvents(root2, featureId);
  ...
  const result = await resolver({ root: root2, featureId, expectedRevision, host, credential: resolvedCredential, interaction, state });
...
async function answerFromHostEvents(input) {
  const { root: root2, featureId, expectedRevision, host } = input;
  const state = await readState(root2, featureId);
  if (state.revision !== expectedRevision) {
    throw new DevFlowError("STATE_REVISION_CONFLICT", "state revision changed", { currentRevision: state.revision });
  }
  ...
  if (!selected) {
    ...
    throw new DevFlowError("INTERACTION_EVENT_MISSING", "\u6CA1\u6709\u627E\u5230\u5448\u73B0\u95EE\u9898\u4E4B\u540E\u3001\u6765\u81EA\u5F53\u524D\u5BBF\u4E3B\u7684\u672A\u6D88\u8D39\u7528\u6237\u6D88\u606F\u3002", {
      userMessage: "\u5F53\u524D\u95EE\u9898\u4ECD\u672A\u56DE\u7B54\u3002",
      cause: health2 === "fresh" ? "\u5BBF\u4E3B hook \u5065\u5EB7\u4F46\u672A\u6355\u83B7\u5230\u5448\u73B0\u540E\u7684\u7528\u6237\u6D88\u606F\uFF0C\u6216\u8BE5\u6D88\u606F\u5DF2\u88AB\u6D88\u8D39\u3002" : health2 === "stale" ? "\u5BBF\u4E3B hook \u7684\u6700\u8FD1\u53EF\u4FE1\u4FE1\u53F7\u5DF2\u8FC7\u671F\uFF0C\u4E8B\u4EF6\u6355\u83B7\u4E0D\u53EF\u4FE1\u3002" : "\u5C1A\u672A\u53D1\u73B0\u5F53\u524D\u5BBF\u4E3B\u7684\u53EF\u4FE1 hook \u5065\u5EB7\u4FE1\u53F7\uFF0C\u4E8B\u4EF6\u6355\u83B7\u4E0D\u53EF\u4FE1\u3002",
      recoveryKind: "retry",
      recoveryInstruction: health2 === "fresh" ? "\u8BF7\u7528\u6237\u91CD\u65B0\u53D1\u9001\u4E00\u6B21\u5B8C\u6574\u56DE\u7B54\uFF1B\u82E5\u53CD\u590D\u5931\u8D25\u518D\u8FD0\u884C doctor \u68C0\u67E5 UserPromptSubmit hook\u3002" : "\u5148\u8FD0\u884C dev_flow_doctor \u6062\u590D\u5BBF\u4E3B hook \u5E76\u91CD\u65B0\u5F00\u542F\u4F1A\u8BDD\uFF0C\u518D\u8BA9\u7528\u6237\u91CD\u65B0\u56DE\u7B54\uFF1Bhook \u6062\u590D\u524D\u56DE\u7B54\u65E0\u6CD5\u843D\u8D26\u3002",
      retryOriginal: true,
      health: health2
    });
  }
...
import { randomUUID as randomUUID17, createHash as createHash33 } from "node:crypto";
import { access as access4, mkdir as mkdir15, open as open11, readdir as readdir8, readFile as readFile23, rename as rename10, rm as rm3, writeFile as writeFile4 } from "node:fs/promises";
...
async function readProjectConfig(root2) {
  try {
    const raw = await readFile23(path27.join(devFlow(root2), "project.json"), "utf8");
    const value = JSON.parse(raw);
...
async function initProject(root2, config) {
  validateProjectConfig(config);
  await mkdir15(devFlow(root2), { recursive: true });
  try {
    const existing = JSON.parse(await readFile23(path27.join(devFlow(root2), "project.json"), "utf8"));
  ...
  await writeAtomic2(path27.join(devFlow(root2), "project.json"), config);
}
async function updateProjectConfig(root2, config, expectedSha256) {
  ...
  try {
    const file = path27.join(devFlow(root2), "project.json");
    let raw;
    try {
      raw = await readFile23(file, "utf8");
    } catch {
      throw new DevFlowError("PROJECT_NOT_INITIALIZED", "run dev_flow_init_project first");
    }
    const previousSha256 = createHash33("sha256").update(raw).digest("hex");
    ...
    const nextRaw = await readFile23(file, "utf8");
    return { config, previousSha256, sha256: createHash33("sha256").update(nextRaw).digest("hex"), impact, affectedEvidence };
  } finally {
    await release();
  }
}
async function writeAtomic2(file, value) {
  ...
  const handle = await open11(temp, "w");
  const payload = file.endsWith(`${path27.sep}state.json`) && value && typeof value === "object" && value.schemaVersion === 6 ? (() => {
    const copy = { ...value };
...
`);
  ...
  await rename10(temp, file);
  const directory = await open11(path27.dirname(file), "r");
  try {
    await directory.sync();
  } finally {
    await directory.close();
...
async function prepareStatusProjection(root2, state, revision) {
  ...
  if (state.mode !== "routed" || !state.route || !state.classification) {
    ...
    const file2 = path27.join(features2(root2), state.featureId, status.path);
    state.artifacts.status = { ...status, sha256: createHash33("sha256").update(contents2).digest("hex") };
    return async () => {
...
`;
  const file = path27.join(features2(root2), state.featureId, status.path);
  state.artifacts.status = { ...status, sha256: createHash33("sha256").update(contents).digest("hex") };
  return async () => {
    await writeFile4(file, contents);
  };
}
async function lock(root2, featureId, operation) {
  const directory = path27.join(devFlow(root2), ".lock");
  const started = Date.now();
  await mkdir15(devFlow(root2), { recursive: true });
  while (true) {
    try {
      await mkdir15(directory);
      await writeFile4(path27.join(directory, "owner.json"), JSON.stringify({ pid: process.pid, hostname: hostname2(), acquiredAt: (/* @__PURE__ */ new Date()).toISOString(), featureId, operation }));
      return async () => {
      ...
      try {
        const owner = JSON.parse(await readFile23(path27.join(directory, "owner.json"), "utf8"));
...
async function readState(root2, featureId) {
  try {
    const raw = JSON.parse(await readFile23(statePath(root2, featureId), "utf8"));
    const state = raw?.schemaVersion === 6 ? await hydrateFeatureState(root2, raw) : (() => {
...
async function readActive(root2) {
  let raw;
  try {
    raw = await readFile23(activePath(root2), "utf8");
  } catch (error) {
    if (error.code === "ENOENT") return void 0;
...
async function appendEvent(root2, id, revision, type, data) {
  const eventSequence = await nextFeatureEventSequence(root2, id);
  const handle = await open11(eventPath(root2, id), "a");
  try {
...
async function stateFileSha256(root2, featureId) {
  const contents = await readFile23(statePath(root2, featureId));
  return createHash33("sha256").update(contents).digest("hex");
}
...
async function readFeatureEvents(root2, id) {
  const result = await readSegmentedFeatureEvents(root2, id);
...
async function startFeature(root2, input, options = {}) {
  ...
  const id = input.featureId ?? randomUUID17();
  ...
  try {
    ...
    const directory = path27.join(features2(root2), id);
    ...
    try {
      ...
      const state = {
        ...
        lastUpdatedBy: { host: input.host, pluginVersion: "6.0.0" }
      };
      ...
      const persisted = await persistableFeatureState(root2, state);
      await writeAtomic2(statePath(root2, id), persisted);
...
async function mutatePreparedLocked(root2, id, expectedRevision, operation, prepare, options = {}) {
  ...
  state.revision += 1;
  const nextSequence = await nextFeatureEventSequence(root2, id);
  for (const interaction of Object.values(state.interactions ?? {})) {
  ...
  try {
    await runBoundedEvidenceMaintenance(root2, id, state);
  } catch {
...
async function pauseFeature(root2, id, expectedRevision, reason, host) {
  ...
  const paused = await mutate(root2, id, expectedRevision, "feature-paused", (state) => {
  ...
  await maybeSealFeatureEvents(root2, id);
  return paused;
}
...
function validateRecoveryTransaction(value) {
  const transaction = value;
  if (transaction?.schemaVersion !== 1 || typeof transaction.transactionId !== "string" || !transaction.transactionId || !isRecoveryPhase(transaction.phase) || typeof transaction.featureId !== "string" || !transaction.featureId || typeof transaction.stateSha256 !== "string" || !transaction.stateSha256 || typeof transaction.recoveredTo !== "string" || !path27.isAbsolute(transaction.recoveredTo) || typeof transaction.reason !== "string" || typeof transaction.userEvidence !== "string" || transaction.host !== "claude" && transaction.host !== "codex" || typeof transaction.at !== "string" || transaction.activeSha256 !== void 0 && typeof transaction.activeSha256 !== "string") {
  ...
  if (path27.basename(transaction.featureId) !== transaction.featureId || transaction.featureId === "." || transaction.featureId === "..") {
...
function validateRecoveryLocation(root2, transaction) {
  const recoveredRoot = path27.join(devFlow(root2), "recovered");
  const relative = path27.relative(recoveredRoot, transaction.recoveredTo);
  if (!relative || relative.startsWith("..") || path27.isAbsolute(relative) || path27.basename(relative) !== relative) {
...
async function readRecoveryTransaction(root2) {
  let raw;
  try {
    raw = await readFile23(recoveryTxnPath(root2), "utf8");
  } catch (error) {
    if (error.code === "ENOENT") return void 0;
...
async function fileSha256(file) {
  return createHash33("sha256").update(await readFile23(file)).digest("hex");
}
...
async function recoveryEventExists(root2, transactionId) {
  try {
    return (await readFile23(recoveryEventsPath(root2), "utf8")).split("\n").filter(Boolean).some((line) => {
      try {
...
async function appendRecoveryEvent(root2, transaction) {
  ...
  const handle = await open11(recoveryEventsPath(root2), "a");
  try {
...
async function resumeRecovery(root2, transaction) {
  const sourceDir = path27.join(features2(root2), transaction.featureId);
  if (transaction.phase === "prepared") {
    ...
    if (sourceExists) await rename10(sourceDir, transaction.recoveredTo);
  ...
  if (transaction.phase === "directory-moved") {
    if (transaction.activeSha256) {
      if (await pathExists4(activePath(root2))) {
        ...
        await rename10(activePath(root2), path27.join(transaction.recoveredTo, "active.json"));
      }
    } else {
      const active = await readActive(root2);
...
async function readRollbackJournalPresence(root2, featureId) {
  let raw;
  try {
    raw = await readFile23(rollbackTxnPath2(root2, featureId), "utf8");
  } catch (error) {
    if (error.code === "ENOENT") return void 0;
    throw new DevFlowError("ROLLBACK_TRANSACTION_UNREADABLE", "rollback transaction journal cannot be read", {
      recoveryHint: "Run dev_flow_doctor; the workspace may be mid-rollback \u2014 do not hand-edit .dev-flow"
...
async function assertNoOpenRollbackTransaction(root2, allow) {
  let entries;
  try {
    entries = await readdir8(features2(root2), { withFileTypes: true });
  } catch (error) {
    if (error.code === "ENOENT") return;
    throw error;
  }
...
async function recoverCorruptFeature(root2, input) {
  ...
  if (path27.basename(input.featureId) !== input.featureId || input.featureId === "." || input.featureId === "..") throw new DevFlowError("INVALID_FEATURE_ID", "recovery featureId must name one feature directory");
  ...
  try {
    ...
    let digest14;
    try {
      digest14 = await stateFileSha256(root2, input.featureId);
    } catch {
    ...
    if (digest14 !== input.stateSha256) throw new DevFlowError("RECOVERY_DIGEST_MISMATCH", "stateSha256 does not match current corrupt state", { currentDigest: digest14, recoveryHint: "Re-run dev_flow_doctor and use the reported stateSha256" });
    try {
    ...
    const recoveredDir = path27.join(devFlow(root2), "recovered", `${input.featureId}-${timestamp}`);
    await mkdir15(path27.join(devFlow(root2), "recovered"), { recursive: true });
    const prepared = {
      schemaVersion: 1,
      transactionId: randomUUID17(),
      phase: "prepared",
      featureId: input.featureId,
      stateSha256: digest14,
...
var lifecycles, unitStatuses, interactionKinds, delay, devFlow, features2, statePath, eventPath, activePath, recoveryTxnPath, recoveryEventsPath, rollbackTxnPath2;
var init_state_store = __esm({
  "plugins/dev-flow/src/core/state-store.ts"() {
    ...
    devFlow = (root2) => path27.join(root2, ".dev-flow");
    features2 = (root2) => path27.join(devFlow(root2), "features");
    statePath = (root2, id) => path27.join(features2(root2), id, "state.json");
    eventPath = (root2, id) => path27.join(features2(root2), id, "events.jsonl");
    activePath = (root2) => path27.join(devFlow(root2), "active.json");
    recoveryTxnPath = (root2) => path27.join(devFlow(root2), "recovery-transaction.json");
    recoveryEventsPath = (root2) => path27.join(devFlow(root2), "recovery-events.jsonl");
    rollbackTxnPath2 = (root2, featureId) => path27.join(features2(root2), featureId, "rollback-transaction.json");
  }
});
...
async function assertArtifactCurrent(root2, id, state, kind) {
  ...
  const contents = await readFile24(path28.join(featureDirectory6(root2, id), normalizeUnicode(artifact.path)), "utf8");
  if (hash2(contents) !== artifact.sha256) throw new DevFlowError("ARTIFACT_INTEGRITY_FAILED", kind);
  return contents;
}
async function readArtifactText(root2, id, artifactPath) {
  return readFile24(path28.join(featureDirectory6(root2, id), normalizeUnicode(artifactPath)), "utf8");
}
async function scaffoldArtifact(root2, id, expectedRevision, kind) {
  ...
  const target = path28.join(featureDirectory6(root2, id), filename);
  const content = template(state, id, kind);
  await writeFile5(target, content, { flag: "wx" }).catch(async (error) => {
    if (error.code !== "EEXIST") throw error;
  });
  const contents = await readFile24(target, "utf8");
  return mutate(root2, id, expectedRevision, "artifact-scaffolded", (current) => {
    current.artifacts[kind] = { path: filename, sha256: hash2(contents) };
  });
}
...
async function validatePlanFromMarkdown(root2, id, artifactKind) {
  const state = await readState(root2, id);
  ...
  const compilation = await compileArtifactPlanFromMarkdown(root2, id, state, {
    artifactKind,
...
async function recordArtifactFromMarkdown(root2, id, expectedRevision, artifactKind, options = {}) {
  if (artifactKind !== "requirements" && artifactKind !== "implementation-plan") {
  ...
  const state = await mutatePrepared(root2, id, expectedRevision, "artifact-recorded-with-trace", async (current, nextStateRevision) => {
    ...
    const compilation = await compileArtifactPlanFromMarkdown(root2, id, current, { artifactKind, nextStateRevision });
    const compile = compilation.result;
    if (compile.ok === false || compile.ledger === void 0 || compilation.input === void 0) {
    ...
    const executionVerificationRefs = Object.values(ledger.nodes).filter((node) => node.status === "current" && node.kind === "implementation-unit").flatMap((node) => node.forwardVerification);
    const executionSemanticBasisHash = hash2(JSON.stringify({
      nodes: executionNodes,
      verificationCommandHashes: verificationCommandHashesForRefs(config, executionVerificationRefs)
    }));
    ...
    const traceChanged = compilation.semanticSha256 !== void 0 && compilation.input.currentLedger ? JSON.stringify(compilation.input.currentLedger.nodes) !== JSON.stringify(ledger.nodes) || JSON.stringify(compilation.input.currentLedger.edges) !== JSON.stringify(ledger.edges) : true;
...
var names, hash2, featureDirectory6, traceArtifactKinds, traceArtifactKindList, artifactInvalidations;
var init_artifacts = __esm({
  "plugins/dev-flow/src/core/artifacts.ts"() {
    ...
    hash2 = (value) => createHash34("sha256").update(value).digest("hex");
    featureDirectory6 = (root2, id) => path28.join(root2, ".dev-flow", "features", id);
    traceArtifactKinds = /* @__PURE__ */ new Set(["requirements", "implementation-plan"]);
    traceArtifactKindList = /* @__PURE__ */ new Set(["requirements", "implementation-plan"]);
...
import { lstat as lstat8, readFile as readFile25, writeFile as writeFile6 } from "node:fs/promises";
import path29 from "node:path";
import { promisify as promisify6 } from "node:util";
var run5 = promisify6(execFile6);
var digest13 = (value) => createHash35("sha256").update(value).digest("hex");
...
function normalizePath2(value) {
  ...
  if (!normalized || path29.posix.isAbsolute(normalized) || normalized.startsWith("../") || normalized === ".." || normalized.startsWith(".dev-flow/") || normalized !== slashPath) {
...
async function assertImplementationFilesExist(root2, files) {
  ...
  for (const file of files) {
    try {
      await lstat8(path29.join(root2, file));
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
      missing.push(file);
    }
...
async function fileHash(root2, file) {
  try {
    return digest13(await readFile25(path29.join(root2, file)));
  } catch (error) {
...
async function assertPlainFile(root2, file) {
  const metadata = await lstat8(path29.join(root2, file));
...
async function createDeliverySnapshot(root2, featureId, state, config) {
  ...
  const relativeDirectory2 = path29.posix.join(".dev-flow", "features", featureId);
  const patchFilename = "\u4EA4\u4ED8\u5FEB\u7167.patch";
  const manifestFilename = "\u4EA4\u4ED8\u5FEB\u7167\u6587\u6863.md";
  const patchPath = path29.posix.join(relativeDirectory2, patchFilename);
  const manifestPath2 = path29.posix.join(relativeDirectory2, manifestFilename);
  const patch = patches.filter(Boolean).join("\n");
  const patchHash = digest13(patch);
  await writeFile6(path29.join(root2, patchPath), patch, "utf8");
  ...
  const manifestHash = digest13(manifest);
  await writeFile6(path29.join(root2, manifestPath2), manifest, "utf8");
  return {
...
async function captureAutomaticCheckpoint(root2, featureId, expectedRevision, stage, reason = "stage-boundary") {
  ...
  const checkpoint = {
    checkpointId: `AUTO-${randomUUID18()}`,
    stage,
    capturedAt,
    fingerprint: fingerprint2,
...
async function recordStep(root2, id, expectedRevision, step, evidence) {
  ...
  const next = await mutate(root2, id, expectedRevision, "step-recorded", async (state) => {
    ...
    if (step === "code_review") {
      ...
      const gov = state.governance ?? EMPTY_GOVERNANCE_LEDGER;
      const claimId = `CLAIM-${createHash36("sha256").update(`review-complete|code_review|${fingerprint2}`).digest("hex").slice(0, 16)}`;
      const claims = [...gov.claims];
      if (!claims.some((claim) => claim.recordId === claimId)) {
        const baselineRef = await captureEvidenceBaseline(root2, state, config, {
  ...
  await maybeSealFeatureEvents(root2, id);
  const supportsAutomaticCheckpoint = Number(next.schemaVersion) === 4 || Number(next.schemaVersion) === 5 || Number(next.schemaVersion) === 6;
...
async function finalize(root2, id, expectedRevision) {
  ...
  let snapshot;
  const finalized = await mutate(root2, id, expectedRevision, "finalized", async (state) => {
    await assertRequirementsGrillSatisfied(root2, id, state);
    assertVerificationWasNotInvalidated(state);
    state.workspace = reconciledWorkspace;
    const open12 = currentOpenStep(state);
    if (open12 !== "finalize" && !(open12 && qualityExceptionCoversStep(state, open12))) {
...
async function reviewPlanAction(root2, state) {
  ...
  if (gate.status === "ready") {
    const planningEvidence = state.steps.planning?.evidence;
...
async function review(root2, state) {
  ...
  const nonBlocking = current ? current.jobs.flatMap((job) => job.submission?.findings ?? []).filter((finding) => finding.severity !== "blocking") : [];
...
init_decision_interactions();
init_git_reconciliation();
import { lstat as lstat9, readdir as readdir9, readFile as readFile26 } from "node:fs/promises";
...
async function validJson(file) {
  try {
    JSON.parse(await readFile26(file, "utf8"));
    return true;
  } catch {
    return false;
  }
...
async function pointerRecoveryCandidates(root2) {
  try {
    const directory = path30.join(root2, ".dev-flow", "features");
    const entries = await readdir9(directory, { withFileTypes: true });
...
async function collectDoctorReport(root2, pluginRoot, version, tools) {
  ...
  const projectFile = path30.join(root2, ".dev-flow", "project.json");
  ...
  const activeFile = path30.join(root2, ".dev-flow", "active.json");
  ...
  if (activeFeature.present) {
    try {
      ...
      try {
        ...
        let digest14;
        try {
          digest14 = await stateFileSha256(root2, active.featureId);
        } catch {
        }
        if (!digest14) {
          try {
            const raw = await readFile26(path30.join(root2, ".dev-flow", "features", active.featureId, "state.json"));
            digest14 = createHash37("sha256").update(raw).digest("hex");
          } catch {
            digest14 = void 0;
          }
        }
      ...
      if (error.code === "ACTIVE_POINTER_UNREADABLE") {
        let activeSha256;
        try {
          activeSha256 = createHash37("sha256").update(await readFile26(activeFile)).digest("hex");
        } catch {
        }
  ...
  try {
    const featuresDirectory = path30.join(root2, ".dev-flow", "features");
    const entries = await readdir9(featuresDirectory, { withFileTypes: true });
  ...
  if (traceState) {
    try {
      const rootSet = ["workspaceLineage", "interactionLedger", "governanceLedger", "verificationLedger", "repairLedger"].flatMap((key) => traceState.archivedCollections?.[key] ? [traceState.archivedCollections[key]] : []);
      const health2 = await inspectEvidenceStoreHealth(root2, traceState.featureId, { rootSet });
      evidenceStore = health2;
      if (!health2.catalogPresent) {
        add2("EVIDENCE_STORE_EMPTY", "ok", `feature ${traceState.featureId} has no evidence store objects yet`);
      } else {
        add2(
          "EVIDENCE_STORE_CATALOG_VALID",
          "ok",
          `evidence store ${health2.objectCount} objects in ${health2.packCount} packs (hot ${health2.hotPackCount}/${health2.hotRawBytes}B, cold ${health2.coldPackCount}/${health2.coldRawBytes}B)`
        );
      }
      if (health2.integrityIssues.length > 0) {
        add2(
          "EVIDENCE_STORE_INTEGRITY_FAILED",
          "error",
          `evidence store has ${health2.integrityIssues.length} integrity issue(s): ${health2.integrityIssues.map((issue3) => `${issue3.code}:${issue3.packSha256 ?? ""} ${issue3.message}`).join("; ")}`,
          "Stop writing to this feature; doctor \u53EA\u505A\u53EA\u8BFB\u8BCA\u65AD\u3002\u6062\u590D\u5B8C\u6574 catalog/pack \u6216\u4F7F\u7528 recover_corrupt_feature \u9694\u79BB\u635F\u574F feature\uFF0C\u4E0D\u8981\u624B\u6539 .dev-flow"
        );
      } else if (health2.physicalOrphanFileCount > 0 || health2.orphanPackCount > 0) {
        add2(
          "EVIDENCE_STORE_ORPHAN_BACKLOG",
          "warning",
          `evidence store orphan backlog: ${health2.orphanPackCount} unreachable packs and ${health2.physicalOrphanFileCount} physical leftovers; estimated ${health2.backlogRounds} bounded GC round(s)`,
  ...
  const paths = {
    claudeManifest: path30.join(pluginRoot, ".claude-plugin", "plugin.json"),
    codexManifest: path30.join(pluginRoot, ".codex-plugin", "plugin.json"),
    mcp: path30.join(pluginRoot, ".mcp.json"),
    claudeHooks: path30.join(pluginRoot, "hosts", "claude", "hooks.json"),
    codexHooks: path30.join(pluginRoot, "hosts", "codex", "hooks.json"),
    mcpBundle: path30.join(pluginRoot, "dist", "mcp-server.mjs"),
    claudeBundle: path30.join(pluginRoot, "dist", "claude-hook.mjs"),
    codexBundle: path30.join(pluginRoot, "dist", "codex-hook.mjs")
  };
  ...
  try {
    ...
    for (const entry of entries.filter((candidate) => candidate.isDirectory())) {
      try {
        const raw = JSON.parse(await readFile26(path30.join(directory, entry.name, "state.json"), "utf8"));
  ...
  add2(v4Ready ? "V4_READY" : "V4_NOT_READY", v4Ready ? "ok" : "warning", v4Ready ? "\u6CA1\u6709\u672A\u5B8C\u6210\u7684\u65E7\u7248 feature\uFF0C\u53EF\u4EE5\u4F7F\u7528 schema v6" : `\u4ECD\u6709\u672A\u5B8C\u6210\u7684\u65E7\u7248 feature: ${legacyFeatures.join(", ")}`, v4Ready ? void 0 : "\u5148\u4F7F\u7528\u4EA7\u751F\u8BE5\u72B6\u6001\u7684 Dev Flow \u7248\u672C\u5B8C\u6210\u6216\u653E\u5F03\u65E7 feature\uFF0C\u5907\u4EFD .dev-flow\uFF0C\u518D\u4F7F\u7528 6.0 \u91CD\u65B0\u521D\u59CB\u5316\uFF1Bdoctor \u4E0D\u81EA\u52A8\u8FC1\u79FB\u6216\u7EC8\u6B62");
  return {
    version,
    root: root2,
    pluginRoot,
...
function childPath(path32, key) {
  return typeof key === "number" ? `${path32}[${key}]` : `${path32}.${key}`;
}
function issue2(path32, keyword, message, extra = {}) {
  return { path: path32, keyword, message, ...extra };
}
...
function validate(value, schema, path32) {
  if (Object.keys(schema).length === 0) return [];
  const issues = [];
  const expectedType = schema.type;
  if (typeof expectedType === "string" && !matchesType(value, expectedType)) {
    return [issue2(path32, "type", `expected ${expectedType}, got ${typeOf(value)}`)];
  }
  if (Array.isArray(expectedType) && !expectedType.some((candidate) => typeof candidate === "string" && matchesType(value, candidate))) {
    return [issue2(path32, "type", `expected one of ${expectedType.join(", ")}, got ${typeOf(value)}`)];
  }
  if (schema.const !== void 0 && stableJson(value) !== stableJson(schema.const)) {
    issues.push(issue2(path32, "const", `must equal ${stableJson(schema.const)}`));
  }
  if (Array.isArray(schema.enum) && !schema.enum.some((candidate) => stableJson(value) === stableJson(candidate))) {
    issues.push(issue2(path32, "enum", "must be one of the allowed values"));
  }
  if (typeof value === "string") {
    if (typeof schema.minLength === "number" && value.length < schema.minLength) issues.push(issue2(path32, "minLength", `must have length >= ${schema.minLength}`));
    if (typeof schema.pattern === "string") {
      ...
      if (!matches) issues.push(issue2(path32, "pattern", "does not match the required pattern"));
    }
  }
  if (typeof value === "number" && typeof schema.minimum === "number" && value < schema.minimum) {
    issues.push(issue2(path32, "minimum", `must be >= ${schema.minimum}`));
  }
  if (Array.isArray(value)) {
    if (typeof schema.minItems === "number" && value.length < schema.minItems) issues.push(issue2(path32, "minItems", `must contain at least ${schema.minItems} items`));
    if (typeof schema.maxItems === "number" && value.length > schema.maxItems) issues.push(issue2(path32, "maxItems", `must contain at most ${schema.maxItems} items`));
    if (schema.uniqueItems === true) {
      const seen = new Set(value.map(stableJson));
      if (seen.size !== value.length) issues.push(issue2(path32, "uniqueItems", "items must be unique"));
    }
    if (schema.items && typeof schema.items === "object" && !Array.isArray(schema.items)) {
      value.forEach((item, index) => issues.push(...validate(item, schema.items, childPath(path32, index))));
    }
  }
  if (typeof value === "object" && value !== null && !Array.isArray(value)) {
    ...
    for (const key of required) {
      if (!(key in record)) issues.push(issue2(childPath(path32, key), "required", "is required"));
    }
    const additional = schema.additionalProperties;
    for (const [key, item] of Object.entries(record)) {
      if (properties[key]) {
        issues.push(...validate(item, properties[key], childPath(path32, key)));
      } else if (additional === false) {
        issues.push(issue2(childPath(path32, key), "additionalProperties", "unknown field", {
          unknownField: key,
          allowedFields: Object.keys(properties).sort()
        }));
      } else if (additional && typeof additional === "object" && !Array.isArray(additional)) {
        issues.push(...validate(item, additional, childPath(path32, key)));
      }
    }
    ...
    if (propertyNames) {
      for (const key of Object.keys(record)) issues.push(...validate(key, propertyNames, childPath(path32, key)));
    }
  }
  if (Array.isArray(schema.oneOf)) {
    const candidates = schema.oneOf.filter((candidate) => typeof candidate === "object" && candidate !== null && !Array.isArray(candidate));
    const results = candidates.map((candidate) => validate(value, candidate, path32));
    ...
    if (valid.length !== 1) {
      ...
      issues.push(...best);
      issues.push(issue2(path32, "oneOf", "must match exactly one schema"));
    }
  }
  return issues;
}
...
function pluginRootForDoctor(fallbackRoot) {
  try {
    const moduleDirectory = path31.dirname(fileURLToPath(import.meta.url));
    return path31.basename(moduleDirectory) === "dist" ? path31.resolve(moduleDirectory, "..") : path31.resolve(moduleDirectory, "../..");
  } catch {
    return fallbackRoot;
  }
}
...
var traceArtifactKinds2 = ["requirements", "implementation-plan"];
...
var toolSchemas = {
  ...
  dev_flow_validate_plan: {
    ...
    inputSchema: featureMutation({ kind: { enum: traceArtifactKinds2 } }, ["kind"]),
    annotations: { readOnlyHint: true }
  },
...
async function dispatch(root2, name, a, ports2) {
  ...
  switch (tool) {
    case "dev_flow_init_project": {
      ...
      return { \u72B6\u6001: "\u5DF2\u521D\u59CB\u5316", \u914D\u7F6E\u8DEF\u5F84: path31.join(root2, ".dev-flow", "project.json"), \u4E0B\u4E00\u6B65: "\u8C03\u7528 dev_flow_start \u5F00\u59CB\u4E00\u4E2A\u9700\u6C42\u3002" };
    }
    case "dev_flow_update_project": {
      const result = await updateProjectConfig(root2, a.config, a.expectedSha256);
      return { \u72B6\u6001: "\u5DF2\u66F4\u65B0", \u914D\u7F6E\u8DEF\u5F84: path31.join(root2, ".dev-flow", "project.json"), previousSha256: result.previousSha256, sha256: result.sha256, \u53D8\u5316: result.impact, \u53D7\u5F71\u54CD\u8BC1\u636E: result.affectedEvidence, \u4E0B\u4E00\u6B65: "\u65B0\u589E\u6216\u6269\u5145\u9A8C\u8BC1\u80FD\u529B\u53EF\u7EE7\u7EED\u5F53\u524D\u4EFB\u52A1\uFF1B\u88AB\u5F15\u7528\u547D\u4EE4\u53D8\u5316\u7684 Trace/RU \u9700\u8981\u6309\u9519\u8BEF\u63D0\u793A\u91CD\u65B0\u767B\u8BB0\u3002" };
    }
    ...
    case "dev_flow_revise_plan": {
      const result = await revisePlanFromMarkdown(root2, a.featureId, a.expectedRevision, a.host);
      return elicitAndAnswer(pipelinePorts, result, {
        root: root2,
        featureId: a.featureId,
        host: a.host,
    ...
    case "dev_flow_record_artifact": {
      const state = await readState(root2, a.featureId);
      const traceKind = a.kind === "requirements" || a.kind === "implementation-plan";
      if (traceKind && traceEnforcementRequired(state.route, state.classification.controls)) {
        return recordArtifactFromMarkdown(root2, a.featureId, a.expectedRevision, a.kind);
      }
      return recordArtifact(root2, a.featureId, a.expectedRevision, a.kind);
    }
    case "dev_flow_validate_plan": {
      return validatePlanFromMarkdown(root2, a.featureId, a.kind);
    }
    ...
    case "dev_flow_start_review_execution": {
      ...
      if (host !== "claude") {
        try {
          ...
          sampleReview = (job) => ports2.sampleReview(job);
        } catch (error) {
      ...
      return startReviewExecution(root2, input.featureId, input.expectedRevision, input.batchId, host, input.executionRequestId, { sampleReview });
    }
    case "dev_flow_complete_review_execution": {
      const input = reviewMutationInput(a);
      return completeReviewExecution(root2, input.featureId, input.expectedRevision, input.batchId, input.executionRequestId);
    }
    ...
    case "dev_flow_doctor":
      return collectDoctorReport(root2, pluginRootForDoctor(root2), "6.0.0", publicTools);
...
function isFeatureState(value) {
  ...
  return Boolean(value && typeof value === "object" && !Array.isArray(value) && schemaVersion === 6 && typeof value.featureId === "string" && typeof value.revision === "number" && typeof value.mode === "string");
}
  ...
  try {
    ...
    if (message.method === "initialize") {
      ...
      protocolResult(message.id, {
        ...
        serverInfo: { name: "dev-flow", version: "6.0.0" },
        capabilities: { tools: {} },
