...
- `/finish` 先调用 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs next [feature-id]`（在仓库根目录执行），一次推进一个 blocker；必须有新鲜验证证据。1.0.1 中 logic-complete 后可 Git，`not now` 不阻塞，compact 含 untracked 删除时须 exact token。
