...
3. 分类输入齐备后**同回合**运行 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs start <feature-id> …`（在仓库根目录执行）：XS/S 不传 execution；M/L 传 `--execution light|standard`，standard 再传 `--requirements`。dirty workspace 明确 `--existing-diff unrelated|in-scope --reason`。需要预览时先 `--dry-run`；真实 start 原子写授权/status，不得把写权限推到下一轮。
