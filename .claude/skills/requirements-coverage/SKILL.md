...
4. 标记状态。
5. 输出阻塞缺口和建议修补点。
6. 标准 M/L 中，如果计划审查门禁被风险维度触发，把结论传给 `plan-review`；否则传给下一道已触发门禁或实现前确认。L 级必须保存到 `<FEATURE_ROOT>/<feature-id>/requirements-coverage.md`。
7. 保存或输出结论后更新 `<FEATURE_ROOT>/<feature-id>/status.md`，记录覆盖门禁状态、`dev_flow_status` 和下一步。
8. 如果存在 context manifest，把覆盖矩阵追加到 `context/review.jsonl` 和 `context/verify.jsonl`；轻量 M 没有落盘资产时不为此新建清单。
...
- 无阻塞时，交给下一道已触发门禁；如果计划审查被触发，则 `Next skill: plan-review`，`Auto-continue: yes`。
...
- 无其它前置检查门禁触发：`Next skill: subagent-driven-development` 或 `executing-plans`，并在实现前确认
