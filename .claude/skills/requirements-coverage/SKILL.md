...
6. L 级 `full` 形态必须保存到 `<FEATURE_ROOT>/<feature-id>/requirements-coverage.md`；`light` 形态可以只把结论写入对话和 `status.md`，或内嵌在计划的 `## Requirements coverage` 章节。
...
输出顺序：一句话结论（通过/有阻塞缺口/只能部分判断）、统计（各状态数量）、阻塞项、覆盖矩阵、建议修订、`[HANDOFF]`。
