...
先读取项目适配层。Claude 环境默认读取 `.claude/rules/project-workflow.md`，从中获取 `<FEATURE_ROOT>` 和验证配置；写文件前再展开为真实路径。
...
7. 保存或输出结论后更新 `<FEATURE_ROOT>/<feature-id>/status.md`，记录覆盖门禁状态和下一步。
