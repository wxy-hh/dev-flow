...
- **severity 识别属于本 skill**：`MISSING`/`CONFLICT`/`OUT_OF_SCOPE` 等阻塞态由本 skill 判定；CLI 不扫描自然语言、不自动 promote。light 执行中一旦出现阻塞缺口，本 skill 必须先调用 `dev-flow-status promote-gate requirements_coverage --to full --reason <text>`，再写 full 资产；即使随后修复，也保留 full 证据便于审计。
...
7. 保存或输出结论后用 `dev-flow-status complete-gate` 更新门禁：`full` 以独立 coverage 资产为准；`light` 且证据在计划内嵌章节时必须写入 `gate_evidence.requirements_coverage`（`path` + 唯一 `heading`）。`assets` 只含真实 path，禁止 `path#heading`。轻量 M 没有落盘资产时不为 coverage 新建记录。
