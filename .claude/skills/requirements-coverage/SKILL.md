...
6. L 级 `full` 形态必须保存到 `<FEATURE_ROOT>/<feature-id>/requirements-coverage.md`；`light` 形态可以只把结论写入对话和 `status.md`。
7. 保存或输出结论后更新 `status.md`，记录覆盖门禁状态、`dev_flow_status` 和下一步；把覆盖矩阵路径追加到 `assets`（`kind: "review"`）。轻量 M 没有落盘资产时不为 coverage 新建记录。
