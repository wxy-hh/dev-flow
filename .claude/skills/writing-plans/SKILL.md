...
如果需求边界没有确认，先返回 `req-probe` 或 `openspec`，不要用猜测补计划。标准 M/L 进入本技能前必须能在 `<FEATURE_ROOT>/<feature-id>/status.md` 的 `dev_flow_status.human_gates.requirement_confirmation.status` 看到 `confirmed`；如果该字段缺失、为 `pending`，或只有自然语言“等待确认”，必须输出 `[HUMAN GATE:requirement_confirmation]` 并停止，不写实现计划、context manifest 或后续门禁资产。
...
3. 更新 `<FEATURE_ROOT>/<feature-id>/status.md`，记录 `Current gate: writing-plans`、完成资产、`dev_flow_status` 和下一步；标准 M/L 同时把 `human_gates.implementation_approval.required` 写为 `true`、`status` 写为 `pending`。
4. 简短说明任务数量、关键风险、命中的 scoped specs 和需要用户确认的点。
5. `[HANDOFF]` 交接块。

标准 M/L 中，如果需求覆盖门禁被风险维度触发，自动交给 `requirements-coverage`；否则交给 `plan-review`。标准 M/L 的计划审查至少为 `light`，不得直接从 `writing-plans` 进入 `subagent-driven-development` 或 `executing-plans`。
