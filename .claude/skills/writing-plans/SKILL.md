...
如果需求边界没有确认，不要用猜测补计划：已有完整需求文档时返回 `grillme`；需求模糊或没有文档时返回 `req-probe`（项目明确采用 OpenSpec 时返回 `openspec`）。标准 M/L 进入本技能前必须已有分类回合通过 `init --entry-gate writing-plans`（或前序路线）创建的 `status.md`；缺失时返回 dev-flow 补做 init，不在本技能临时建状态。如果用户刚在当前对话中明确确认需求基线且没有未决问题，但 gate 仍为 pending，先用 `confirm-human requirement_confirmation --status confirmed` 写入原话 evidence；其他 pending 场景必须输出 `[HUMAN GATE:requirement_confirmation]` 并停止，不写实现计划或后续门禁资产。
