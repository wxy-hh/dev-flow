...
按 `dev-flow/references/protocol.md` 的资产读取优先级；当前 feature 目录的约定文件另见 `需求说明书.md`、`实现计划.md`、`requirements-coverage.md`、`rollback-units.md`。无明确 feature 时查找最近修改的 `<FEATURE_ROOT>/*/实现计划.md`。
...
- `.status` 为 `confirmed`；用户说“接受风险并继续”时也记录为 `confirmed`，并把原话保存在 evidence，required HUMAN GATE 不使用 `skipped`。
