...
所有 SDD 产物必须写入 feature-scoped workspace（`sdd-workspace <feature-id>`），禁止读写裸 `.claude/runtime/sdd/` 下的 legacy shared 文件。
...
```bash
<SKILL_ROOT>/executing-plans/scripts/sdd-checkpoint <feature-id> init --plan <PLAN_FILE>
<SKILL_ROOT>/executing-plans/scripts/sdd-checkpoint <feature-id> show
```

plan fingerprint stale 时先 `refresh-plan`，再继续调度。

### 1. 调度可并行任务

```bash
<SKILL_ROOT>/executing-plans/scripts/task-schedule <PLAN_FILE> --feature <feature-id> --json
```

只派发 `ready` 列表中的任务；`Writes` 重叠或缺少 `Writes` 的任务不得并行。Phase 0 独占，集成检查通过后再派叶子任务。命令名是 `task-schedule`，不要使用已废弃的 `sdd-schedule`。`--feature` 会从 checkpoint 读取 done 列表；也可显式传 `--done 1,2,3`。

### 2. 生成任务简报

```bash
<SKILL_ROOT>/executing-plans/scripts/task-brief <PLAN_FILE> <TASK_NUMBER> --feature <feature-id>
```
...
实现子代理输入：任务简报路径、当前任务的 rollback unit 或需要补齐的回撤字段、需求编号和覆盖矩阵中对应行、必要的接口/上下文补充、报告文件路径（建议 `<SDD_WORKSPACE>/reports/task-N-impl.md`）；要求写代码、运行覆盖验证、提交或报告 diff，并在报告中写明回撤边界。模板：[../implementer-prompt.md](../implementer-prompt.md)。

派发前：

```bash
<SKILL_ROOT>/executing-plans/scripts/sdd-checkpoint <feature-id> mark-progress <TASK_NUMBER>
```
...
```bash
<SKILL_ROOT>/executing-plans/scripts/review-package <BASE> <HEAD> --feature <feature-id>
```
...
1. 标记完成并写入 checkpoint / progress：

```bash
<SKILL_ROOT>/executing-plans/scripts/sdd-checkpoint <feature-id> mark-done <TASK_NUMBER> --base <BASE> --head <HEAD>
```
...
能用文件交接就不要粘贴大段文本：任务简报（`task-brief --feature` 生成）、实现报告（实现子代理写入 feature SDD `reports/`）、审查包（`review-package --feature` 生成）、进度台账（`sdd-checkpoint` → `progress.md`）、回撤证据（写回 rollback unit 清单）。降低上下文占用，也方便从压缩或中断后恢复。
