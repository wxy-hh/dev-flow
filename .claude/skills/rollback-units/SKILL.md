...
在实现前使用。输入是需求、实现计划、需求覆盖矩阵、计划审查结论和已知代码范围。输出每个任务的预期回撤边界。L 级设计模式必须保存到 `<FEATURE_ROOT>/<feature-id>/rollback-units.md`。
...
字段要求：`Files` 列出主要文件或文件组，不要只写"相关文件"；`Produces`/`Consumed by` 写清接口、组件、状态、路由、配置或数据结构及其依赖方；`Commit / Diff range` 实现前可写 `pending`，实现后必须补为 commit、range、diff 文件或等价描述；`Revert command or patch strategy` 写明无 commit 时如何应用或反向应用 patch；`Revert order` 有依赖时必须写顺序；`Post-revert verification` 写具体命令或人工检查点。
...
输出任何落盘报告后，同步更新 `status.md` 和 `dev_flow_status`，把 `rollback-units.md` 追加到 `assets`（`kind: "review"`）。
...
审计模式通过后进入代码审查；审计模式是实现后 `code-review` 前的强制闭环，不得只在报告里建议"稍后补齐"。`Current gate: rollback-units-audit`，`Next skill: code-review`，`Auto-continue: yes`。
