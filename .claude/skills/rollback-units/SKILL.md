...
- **severity 识别属于本 skill**：无法形成单一回撤边界、缺回撤顺序、共享接口无回撤后验证等阻塞态由本 skill 判定；CLI 不扫描自然语言、不自动 promote。light 执行中一旦产生 CRITICAL/HIGH，必须先调用 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs promote-gate <feature-id> rollback_units --to full --reason <text>`（在仓库根目录执行），再落盘 full 报告；即使随后修复，也保留 full 证据与 disposition。

## 资产读取

按 protocol 的资产读取优先级；额外读取 `<REVIEW_ROOT>/<feature-id>-plan-review.md` / `-code-review.md`。执行后审计读取 git status、相关 diff、commit range、任务报告和进度台账。
