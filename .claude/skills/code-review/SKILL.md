...
`[HANDOFF]` 格式见 `dev-flow/references/protocol.md`。仅当本次审查属于 dev-flow 接管、且 `status.md` 路线中的 `code-review` 门时：无 CRITICAL/HIGH 阻塞项，`Current gate: code-review`，`Next skill: verification-before-completion`，`Auto-continue: yes`；控制者必须在同一会话直接进入 verification，不能把 `/finish` 留给用户记忆。用户明确只要求 code-review、没有 active `status.md`，或审查不属于该路线时，只交付审查结论并停止，不调用 verification 或 `/finish`。存在 CRITICAL/HIGH 时，`Next skill: code-review`，`Auto-continue: no`，`Stop reason` 写明需要修复或用户明确接受风险的问题。
