...
3. 审查输入优先读取用户手动路径和上一步 `[HANDOFF]`；没有时再按 feature 目录自动查找需求、计划、回撤清单和进度台账。
4. 审查重点先看需求覆盖、回归风险、项目规范、回撤完整性和缺失验证。
5. Critical / Important 问题必须先处理，或由用户明确接受风险。
6. 审查前优先读取 `<FEATURE_ROOT>/<feature-id>/status.md`；审查后更新状态文件。
7. L 级功能保存到 `<REVIEW_ROOT>/YYYY-MM-DD-<feature-id>-code-review.md`，并在无阻塞时输出 `[HANDOFF]` 给 `verification-before-completion`。
