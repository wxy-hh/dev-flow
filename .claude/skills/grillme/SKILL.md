...
如果没有发现新分支或阻塞问题，且不是 dev-flow 标准 M/L 或 L 级链路，可以输出 `[HANDOFF]` 给 `writing-plans`：
...
dev-flow 标准 M/L 或任何 L 级链路中，`grillme` 不能 `Auto-continue: yes` 直达 `writing-plans`。必须输出：
...
- <updated requirement or openspec paths>
Auto-continue: no
Stop reason: human requirement confirmation required after grillme
[/HANDOFF]
```
