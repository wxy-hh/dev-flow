...
4. 当前 `git status`、`git rev-parse HEAD`、`git diff --stat` 和本次 diff 涉及的文件。
...
保存验证报告后，同步更新 `<FEATURE_ROOT>/<feature-id>/status.md` 的 `Current gate`、`Completed gates`、`Next action`、`Assets`、`Last updated`、`Base SHA`、`Head SHA`、`Working tree dirty`、`Diff stat hash`、`Last validation at` 和 `Last validation commands`。
