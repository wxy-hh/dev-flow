...
验证通过后按 `dev-flow/references/protocol.md` 输出 `[HANDOFF]`（`Current gate: verification-before-completion`，`Next skill: finishing-a-development-branch`，`Auto-continue: no`，`Stop reason: verification passed; choose branch finishing action`），但不要自动提交、合并、推送或创建 PR。验证失败时同样 `Auto-continue: no`，`Stop reason` 写失败命令和推荐下一步。
