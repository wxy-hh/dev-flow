...
7. 完成后执行 `code-review`、`verification-before-completion` 和 `dev-flow-feature-check --finish`；`webapp-testing: disabled` 时落盘 manual-test。
...
4. 触发了行为验证门禁时，验证报告必须有运行时证据（自动化结果，或手动测试脚本路径 + 步骤 + 预期 + 实测结果）。
