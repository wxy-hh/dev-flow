...
Layer 3 顺序：`complete-verification` → `feature-check --finish` → final assets → **logic-complete（可 Git）** → finalizer dry-run（可选）→ 停等 `compact`/`retain full`/`not now` → `--confirm --inventory`（compact+untracked 另需 `--confirm-untracked`）→ finalized check。`not now` 不阻塞 Git。severity 与 promote 由 skill 判定（见 risk-gates）；CLI 不扫 Markdown。
...
5. 标准 M/L 的需求固化统一按三条路线，并在 `init` 时写入 `--entry-gate`：已有完整但尚未确认的需求文档用 `grillme`；需求模糊或没有文档用 `req-probe`，完成后进入 `grillme`；用户已明确确认需求基线且没有未决问题时用 `writing-plans`，登记该确认后跳过前两者。OpenSpec 仅在用户明确要求或项目策略要求时以 `openspec` 替代 `req-probe`，后续仍交给 `grillme`。
...
普通 XS/S 只需对话内输出结论（规模、拓扑依据、风险标签、证据、路径），并**同回合**运行 `dev-flow-status authorize --level XS|S` 写入有限授权；M/L 与风险 XS/S 用 `dev-flow-status init` 创建 `status.md` 并 `activate`，标准 M/L 同时传所选 `--entry-gate`。status 机器字段禁止手改，一律走 CLI（见 `references/status-cli.md`）。
...
2. 同回合 `init`/`activate` 创建 `status.md`（`profile: "risk-minimal"`，`level: "XS"|"S"`），按标签写 `risk_evidence`。
...
2. 同回合 `init --lightweight-l`/`activate` 创建 `status.md`：`level: "L"`，`profile: "standard"`，`human_gates.requirement_confirmation.required: false`，`implementation_approval.required: true`。
...
M/L 阶段结束时通过 CLI 更新 `status.md` 并追加 `[HANDOFF]`（格式见 `references/protocol.md`）；`Auto-continue: no` 或已输出 HUMAN GATE 时必须停止当前回合，等待用户确认后再把对应 HUMAN GATE 更新为 `confirmed`。
