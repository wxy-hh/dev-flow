...
Layer 3 每次先 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs next [feature-id]`（在仓库根目录执行），只推进它给出的一个 blocker；收尾顺序固定为 verification → feature-check → final assets → **logic-complete（可 Git）** → finalizer dry-run（可选）→ 停等精确回复 → confirm → finalized check。无阻塞交接的 `Auto-continue: yes` 要求控制者在同一会话直接调用 Next skill；code-review → verification → finish 可自动推进至 logic-complete。`not now` 不阻塞 Git。severity 与 promote 由 skill 判定；CLI 不扫 Markdown。
...
分类输入齐备后统一运行 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs start <feature-id> …`（在仓库根目录执行）；XS/S 自动 light，M/L 明确 `--execution`，standard 再声明 requirements 状态。dirty 业务 diff 必须声明 `unrelated|in-scope` 和原因。`start --dry-run` 用于预览同一 policy；低层 `authorize/init/activate` 仅用于恢复兼容。status 机器字段禁止手改，一律走 CLI（见 `references/status-cli.md`）。
...
2. 同回合 `start` 创建 `status.md`（`profile: "risk-minimal"`；M light 派生 `risk-minimal-m`），按标签写 `risk_evidence`；只要派生风险门为 full，必须在输出 implementation approval 前完成 report-mode 风险证据的落盘和登记。仅 `in-scope` retrospective 可无标签复用此状态路线。
3. 先完成派生出的实现前 rollback/security gate；行为验证留在实现后。每个完成 gate 都用 `complete-gate` 更新 `next_action`。
4. 输出 `[HUMAN GATE:implementation_approval]`，列出本次实现范围和明确不做范围后等待用户确认；确认前不得写业务代码。
...
4. 输出 `[HUMAN GATE:implementation_approval]`，列出本次实现范围和明确不做范围；确认前不得写业务代码。用户确认后运行 `confirm-human … --status confirmed`。
