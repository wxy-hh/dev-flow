...
Layer 3 每次先 `node .claude/skills/dev-flow/scripts/dev-flow-status.mjs next [feature-id]`（在仓库根目录执行），只推进它给出的一个 blocker；收尾顺序固定为 verification → feature-check → final assets → **logic-complete（可 Git）** → finalizer dry-run（可选）→ 停等精确回复 → confirm → finalized check。`not now` 不阻塞 Git。severity 与 promote 由 skill 判定；CLI 不扫 Markdown。
