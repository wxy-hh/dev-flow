...
- `code-review` 无 Critical/Important 阻塞项时直接进入验证；有阻塞项时先修复或明确接受风险。
...
15. `rollback_units: full` 时先执行审计，再执行 `code-review`。
16. 按项目适配层运行最强相关验证；运行时行为改动必须有 `webapp-testing` 证据或完整手动测试脚本。
17. 运行 `dev-flow-feature-check --finish`；失败时不得进入"验证通过"分支收尾。
...
轻量 L 只需其中 `status.md`、`code-review`、`verification`，行为改动时另加 `manual-test`。轻量 L 的 `status.md` 中 `requirement_confirmation.required` 为 `false`，只强制 `implementation_approval`；完成检查按此区分，不会因 `level: L` 要求需求确认门禁。
