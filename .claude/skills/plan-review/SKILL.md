...
有 CRITICAL/HIGH 时，停下让用户对每条做出决定：采纳 / 反驳 / 调整。根据用户决定修订原始计划，并更新审查报告里的采纳状态，展示修订点摘要。
...
保存报告后把路径追加到 `status.md` 的 `assets`（`kind: "review"`），更新 `dev_flow_status`；标准 M/L 必须保持 `human_gates.implementation_approval.required: true`，用户确认前保持 `status: pending`。按 `dev-flow/references/protocol.md` 输出 `[HANDOFF]`。
...
- 下一步是 `executing-plans`（实现）：不得 `Auto-continue: yes`。必须先输出 `[HUMAN GATE:implementation_approval]`（"计划审查已完成，请确认是否按当前计划、回撤边界、风险接受情况开始实现。"），再输出 `Next skill: executing-plans`，`Auto-continue: no`，`Stop reason: human implementation approval required after plan-review`。
