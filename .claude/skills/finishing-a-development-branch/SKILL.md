...
`feature.md` 汇总最终需求边界、非目标、Requirement IDs、最终方案、依赖关系和计划偏差；`completion.md` 汇总 gate 结果、代码/安全审查、验证命令与人工实测、accepted risks、commit/PR 和业务 diff fingerprint。不可复用的手测结果写入 `completion.md`，只有明确标记为 `reusable: true` 的脚本保留为长期 `manual-test.md`。
