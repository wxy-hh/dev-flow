...
dev-flow 的项目适配值集中在 `.claude/rules/project-workflow.md`，包括资产目录、运行时台账、feature-id 规约、验证矩阵、scoped specs 和当前项目启用的测试/文档能力。迁移到其他项目时先运行 `/onboard-dev-flow` 生成适配层，不要把旧项目路径和命令散写进各个技能。

XS/S 和默认轻量 M 不额外生成流程产物；轻量 L 与标准 M/L 才维护 `status.md` 的 `dev_flow_status`、`human_gates`、`assets` 列表和必要的局部规范引用。出现 `[HUMAN GATE:*]` 或 `Auto-continue: no` 后必须停止，等待用户后续明确确认。
