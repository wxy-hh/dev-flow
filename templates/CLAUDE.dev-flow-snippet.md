...
dev-flow 的项目适配值集中在 `.claude/rules/project-workflow.md`，包括资产目录、运行时台账、feature-id 规约、验证矩阵和当前项目启用的测试/文档能力。迁移到其他项目时先运行 `/onboard-dev-flow` 生成适配层，不要把旧项目路径和命令散写进各个技能。
