...
test("distributed skills describe dynamic review and finalize contracts", async () => {
  ...
  const finish = await text("plugins/dev-flow/skills/finish/SKILL.md");
  ...
  assert.match(finish, /finalize/);
  assert.match(finish, /完整性|完整/);
  assert.match(routes, /rollback-operability/);
  assert.match(routes, /动态/);
});
...
test("intake and status skills surface the classification level/route to the user", async () => {
  ...
  assert.match(task, /锁定后明确告知用户 XS\/S\/M\/L/);
  assert.match(task, /orderedRoute|完整实际路线/);
  assert.match(status, /完整 `orderedRoute`/);
