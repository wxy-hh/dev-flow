...
export function twoClosureTraceDeltaFor(kind, route) {
  if (route !== "standard-m") throw new Error(`two-closure fixture is only defined for ${route}`);
  if (kind === "requirements") {
    return {
      nodes: [
        { kind: "requirement", id: "REQ-001" },
...
export function appendSecondTraceClosure(markdown, kind, route) {
  if (route !== "standard-m") throw new Error(`second closure is only defined for ${route}`);
  if (kind === "requirements") {
