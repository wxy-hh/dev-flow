...
- **每次发布必须原子发布**：Core、Skills、测试、文档、manifest 与预构建 `dist/` 必须属于同一发布候选，不得只升级 Skills 或只带旧 dist。6.0 的 FeatureState 运行态为 schema v6；4.x/5.x active feature 一律 `UNSUPPORTED_*_SCHEMA`，不迁移。升级前用旧插件收尾或备份 `.dev-flow` 后重新初始化。
