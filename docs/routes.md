...
- **Trace source（1.10.0+）**：standard M/L 的 requirements、implementation plan、coverage matrix、rollback units 必须用 `dev_flow_record_artifact_with_trace` 登记；调用方只提供 REQ/AC、TASK/RU、TEST→AC 等业务关系，不能提交 edges、status、hash 或 pointer。
