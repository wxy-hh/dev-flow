...
test("capability-aware contract separates editable and generated artifacts", () => {
...
- [ ] **步骤 3：实现能力与路线合同。** `normalizeWorkflowCapabilities(undefined)` 返回冻结的零能力副本；`routeDefinitionForFeature` 深拷贝基础合同、应用 capability transition，并验证 editable/generated 不重叠。把 `risk-minimal` 和 `standard-m` 的 `status` 从 `requiredArtifacts/artifactSteps` 移到 `generatedArtifacts/generatedArtifactSteps`。在 contract 中预声明两条 Review 2a transition：standard M 的 plan-review 为 `absent → generated`，standard L 为 `editable → generated`；本阶段 `review: 0` 不触发。
...
- [ ] **步骤 4：实现 renderer 和 artifact 模式。** `artifacts.ts` 对 requirements、implementation-plan、coverage-matrix、rollback-units 调用 `renderArtifactTemplate`；其他既有 artifact 保持现有通用模板，不在本任务重构。scaffold 和 integrity 校验读取 editable/generated 的并集，人工 record generated artifact 仍返回 `GENERATED_ARTIFACT_READ_ONLY`。同步把 `tests/unit/artifacts.test.mjs` 的 risk-minimal 断言更新为 editable `risk-card`、generated `status`。

- [ ] **步骤 5：实现严格锚点解析。** 只接受精确注释格式：
...
前缀与 kind 必须匹配；区块范围从当前声明开始到下一个声明开始，区块正文按原始 UTF-8 计算 SHA-256。零声明、重复 ID、同一前缀错误 kind 均抛 `TRACE_SOURCE_ANCHOR_INVALID`。
...
- [ ] **步骤 2：写精确失效红灯测试。** 构造两个 REQ/AC、两个 TASK/TEST/RU；只改变 `REQ-001` 区块，断言关联 TASK/TEST/RU stale，而 `REQ-002` 的闭包保持 current。

- [ ] **步骤 3：写不变量红灯测试。** 覆盖错误前缀、重复 ID、悬空 parent/covers/verifies、孤儿 TASK、TASK 缺 RU、RU task 不对称、RU DAG 环、未知 command ID、调用方 edges/source/status 字段和 project config digest 变化。另断言 standard L 的 TASK→RU deferred reference 在 partial 模式合法、在 complete 模式返回 `TRACE_GRAPH_INVALID`。
...
- [ ] **步骤 2：写提交点故障红灯测试。** 在三个 `TraceStoreFaultPoint` 和两个 prepared mutation fault point 逐一注入错误；`before-state-commit` 以前 state revision/pointer 不变，`after-snapshot-rename` 允许既有 feature 出现孤儿 snapshot；`after-state-commit` 必须返回 `STATE_COMMITTED_PROJECTION_FAILED`，且 details 精确包含 `{ committed: true, currentRevision }`，不得声称回滚。standard start 在 state 首次提交前失败时必须清理本次新建的 feature 目录和 snapshot，不能留下没有 state 的 feature。
...
另写 standard→light 保留相同 pointer、停止强制；legacy 无 capability state 进入任何现有路径都不被升级门禁锁死。

- [ ] **步骤 4：运行红灯。**

```bash
node --test tests/unit/traceability-store.test.mjs tests/unit/state-store.test.mjs tests/unit/reclassify.test.mjs
```

预期：FAIL，原因是 snapshot store 和 state 字段尚不存在。

- [ ] **步骤 5：实现 snapshot store。** `readTraceability` 校验安全相对路径、文件 hash、schemaVersion、featureId、`pointer.revision === ledger.revision`、`ledger.stateRevision <= state.revision`、pointer/ledger summary 与派生 edges；写入时 `ledger.stateRevision` 必须等于即将提交的 state revision。任一不一致抛 `TRACEABILITY_INTEGRITY_FAILED`。

- [ ] **步骤 6：重构 feature CAS 提交。** 在 `state-store.ts` 增加仅供 Core 使用的 locked prepared mutation helper，使 snapshot 在锁内准备、`state.json` 最后提交。提交前失败保持旧 state；提交后 projection 问题统一抛 `STATE_COMMITTED_PROJECTION_FAILED`，details 为 `{ committed: true, currentRevision, failedProjections }`。

- [ ] **步骤 7：实现 start/reclassify。** 所有新 feature 固定 capability；standard 启动和 light→standard 先准备空 snapshot，再提交 route/pointer。standard 首次 state 提交前失败时，只清理本次调用新建的 feature 目录；不得删除调用前已存在的路径。standard→light 不删 snapshot/pointer。旧 feature 的 capability 始终按零值解释。
...
- [ ] **步骤 4：迁移既有 Core 单元测试。** `artifacts.test.mjs` 保留一条“裸登记必须拒绝”的断言，其余 standard requirements 登记改用 `recordArtifactWithTrace + traceDeltaFor`；requirements-grill 和 human-gates 测试同样使用真实 with-trace 登记，不通过修改 state 绕过。
...
- [ ] **步骤 6：实现原子登记。** 所有 I/O 和 delta 计算在 feature lock 内完成；调用任务 3/4 的纯函数和 snapshot store，不在 `artifacts.ts` 复制图规则或文件原子写入代码。
...
- [ ] **步骤 4：迁移既有 standard 流程测试。** `status-progress`、`risk-evidence` 和 `next-evidence` 中的新 standard M/L feature 必须使用 `traceDeltaFor`、真实 artifact 内容和 `recordArtifactWithTrace` 建立当前 Trace，再测试阶段门禁、风险 evidence 和 next/status 一致性。只有明确标注“legacy compatibility”的用例可以删除 `workflowCapabilities` 模拟旧 feature；不得给普通新 feature 直接写 steps 或 ledger 来绕过 Trace。
...
- [ ] **步骤 1：写 MCP interface 红灯测试。** 断言两个工具被发现、输入严格、get 为只读、非法 source/edge 字段在协议层拒绝、业务错误保留稳定 code。
...
预期：FAIL，至少包含 Trace snapshot 未被 Hook 识别、route helper 不支持 with-trace 或新跨宿主测试文件引用的流程尚未完成。

- [ ] **步骤 4：实现 Hook 和 route helper。** `adapter-policy.ts` 把整个 traceability 子树作为控制路径；helper 必须通过 scaffold、写真实 Markdown、with-trace MCP/Core API 和 recordStep 推进，禁止直接修改 state 或注入 ledger。用任务 1–7 已交付的 API 让步骤 2 的 standard/reclassify/双宿主测试通过。
