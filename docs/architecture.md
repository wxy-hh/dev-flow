...
Dev Flow 2.0 是一个同时服务 Claude Code 与 Codex CLI 的本地插件。两个宿主共享同一套 Skills、MCP 与 policy 合同；宿主只归一化工具事件，Core 才拥有路线、阶段、义务和状态的判断权。
...
`dev_flow_next` 返回 `StageCapabilityView`，说明当前 stage、允许动作、完成条件、待处理义务和恢复动作。阶段内允许等价操作自由排序；只有真实的用户决策、阻塞 finding、语义偏航或系统无法恢复时才返回 `waiting-user`。
