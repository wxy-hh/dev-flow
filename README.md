...
- 写入门禁：`enforcement_mode` + `protected_write_roots` + schema 1 `write-authorization.json`（approved 绑定 approval_basis；见 `dev-flow/references/status-cli.md`）。Edit/Write/Bash 共用同一授权事实。
...
- `/finish`：complete-verification → feature-check → final assets → logic-complete → finalizer dry-run → 停等 `compact` / `retain full` / `not now`；禁止同回合 `--confirm`。`not now` **不阻塞 Git**。
...
- 受管升级：`dev-flow-upgrade --target <abs-path> --check|--apply`；**活动 status 存在时零修改拒绝**；清单 `.claude/dev-flow.manifest.json`。
